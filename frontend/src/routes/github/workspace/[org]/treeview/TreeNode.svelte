...
<div class="select-none">
	<!-- Current Node -->
	<div
		class="group relative flex cursor-pointer items-center space-x-1 rounded-md px-2 py-1 {isSelected
			? 'bg-blue-100 text-blue-900'
			: 'hover:bg-gray-100'} {isDragOver
			? 'border-2 border-dashed border-yellow-400 bg-yellow-100'
			: ''}"
	...
	>
		...
		<button
			class="flex h-4 w-4 items-center justify-center rounded hover:bg-gray-200 {hasChildren
				? 'text-gray-600'
				: 'text-transparent'}"
		...
		<div class="min-w-0 flex-1">
			{#if isEditing}
				<input
					...
					class="w-full rounded border border-blue-300 px-2 py-1 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
				...
				/>
			{:else}
				<span class="text-sm font-medium {getNodeColor(node.type)} block truncate">
					{node.name}
				</span>
			{/if}
		</div>
		...
		<div class="flex items-center space-x-1 opacity-0 transition-opacity group-hover:opacity-100">
			{#if !isEditing}
				...
				{#if node.type === 'workflow'}
					<button
						class="flex h-5 w-5 items-center justify-center rounded text-red-500 hover:bg-red-100 hover:text-red-700"
				...
				{#if node.type === 'folder' && (node.repository || node.metadata?.repository)}
					<button
						class="flex h-5 w-5 items-center justify-center rounded text-orange-500 hover:bg-orange-100 hover:text-orange-700"
					...
					>
						{#if isRepositoryScanning}
							<div class="h-3 w-3 animate-spin rounded-full border-b-2 border-orange-500"></div>
						{:else}
							<svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									stroke-linecap="round"
				...
				<button
					class="flex h-5 w-5 items-center justify-center rounded text-gray-500 hover:bg-gray-200 hover:text-gray-700"
		...
		{#if node.type === 'workflow' && hasSecurityResult}
			<div class="ml-2 flex items-center space-x-1">
				<span
					class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {getSecurityRiskColor(
						securityRiskLevel
					)}"
				...
				{#if securityResult.vulnerability_count > 0}
					<span
						...
						title="{securityResult.vulnerability_count} vulnerabilities found"
					>
		...
		{#if node.metadata && node.metadata.triggers}
			<div class="flex items-center space-x-1">
				{#each node.metadata.triggers.slice(0, 2) as trigger}
					<span
						class="inline-flex items-center rounded-full bg-gray-100 px-1 py-0.5 text-xs font-medium text-gray-600"
					>
						{trigger}
					</span>
				{/each}
