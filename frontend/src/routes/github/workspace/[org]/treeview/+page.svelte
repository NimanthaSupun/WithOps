	...
	function mapGitHubStepsToWorkflowSteps(gitHubRun, workflowSteps) {
		...
		return workflowSteps.map((step, index) => {
			...
			return {
				...step,
				status: matchedStep
					? mapStatusToWorkflowState(matchedStep.conclusion || matchedStep.status)
				...
				if (node.type === 'workflow') {
					...
					const possibleKeys = [
						...
						...(node.metadata?.workflowName
							? [
									`${node.repository}::${node.metadata.workflowName}`,
									`${node.metadata?.repository}::${node.metadata.workflowName}`
								]
							: [])
					];
					...
					for (const [key, githubData] of githubWorkflowMap.entries()) {
						...
						const isMatch =
							...
							(node.metadata?.workflowName === githubData.workflowName &&
			...
			workflowsResult.actions.forEach((action) => {
				...
				if (!workflowMap.has(workflowKey)) {
					workflowMap.set(workflowKey, {
						id: `workflow-${action.repo_name}-${action.workflow_filename}`.replace(
							/[^a-zA-Z0-9-]/g,
							'-'
						),
    ...
    steps:
      ...
      - name: Build
        run: echo "Building ${action.workflow_name}"`,
			...
			console.log(
				`📋 Found ${workflowMap.size} unique REAL workflows:`,
...
`,
	...
	async function loadWorkflowHistory(workflow) {
		try {
			...
			const workflowFilename =
				workflow.metadata?.workflowFilename ||
				pathFilename ||
			...
			console.log(
				`📋 Fetching GitHub Actions history for workflow: ${workflowId} (display name: ${workflow.metadata?.workflowName || workflow.name})`
			);
			...
			const githubResult = await githubClient.getGitHubActionsHistory(
				workflowId,
				orgName,
				repositoryName
			);
			...
			if (
				...
				githubResult.data &&
				githubResult.data.runs &&
				githubResult.data.runs.length > 0
			) {
				...
				console.log(
					`ℹ️ No workflow runs found for ${workflow.name} - this might be a new workflow`
				);
	...
	function findLastSuccessfulRun(runs) {
		return runs.find(
			(run) => mapStatusToWorkflowState(run.conclusion || run.status) === workflowStates.SUCCESS
		);
	}

	function findLastFailedRun(runs) {
		return runs.find(
			(run) => mapStatusToWorkflowState(run.conclusion || run.status) === workflowStates.FAILURE
		);
	}
	...
	function generateMockStages() {
		...
		return stageNames.map((name, index) => ({
			...
			status:
				index < 4
					? workflowStates.SUCCESS
					: Math.random() > 0.3
						? workflowStates.SUCCESS
						: workflowStates.FAILURE,
	...
	async function createNewItem() {
		...
		const newItem = {
			...
			path:
				newItemType === 'workflow'
					? `.github/workflows/${newItemName.trim().replace(/\s+/g, '-').toLowerCase()}.yml`
					: undefined,
			content:
				newItemType === 'workflow'
					? `name: ${newItemName.trim()}
...
jobs:
  build:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Build
        run: echo "Building ${newItemName.trim()}"`
					: undefined,
			metadata:
				newItemType === 'workflow'
					? {
		...
		try {
			await saveTreeData();
			showSaveStatus(
				`${newItemType === 'folder' ? 'Folder' : 'Workflow'} "${newItemName.trim()}" created and saved successfully`,
				true
			);
			...
			showSaveStatus(
				`${newItemType === 'folder' ? 'Folder' : 'Workflow'} created locally but failed to save to database`,
				false
			);
		}
	...
	async function triggerGitHubActions(workflow) {
		...
		const response = await fetch(
			`${API_BASE_URL}/api/github/workspace/${orgName}/actions/trigger`,
			{
				method: 'POST',
				headers: {
			...
			try {
				...
				if (response.ok) {
					...
					if (status.completed) {
						...
						await loadWorkflowHistory(selectedNode);
						showSaveStatus(
							`Workflow completed with status: ${status.result}`,
							status.result === 'success'
						);
						return;
					}
	...
	async function simulateWorkflowExecution(workflow) {
		...
		workflowHistory = { ...workflowHistory };
		showSaveStatus(
			`Workflow "${workflow.name}" completed with status: ${newRun.status}`,
			newRun.status === workflowStates.SUCCESS
		);
	}
	...
	async function showWorkspace(node) {
		...
		showSaveStatus(
			`📁 Workspace: ${node.name} - Use the editor to view and modify workflow files`,
			true
		);
	...
	function deleteWorkflowRun(run) {
		if (
			confirm(
				`⚠️ Delete build ${run.buildNumber}?\n\nThis will permanently remove:\n• Build history\n• Artifacts\n• Logs\n\nThis cannot be undone.`
			)
		) {
...
<div class="min-h-screen bg-gray-50">
	<div class="flex h-screen">
		<!-- Tree Panel -->
		<div class="flex w-1/3 flex-col border-r border-gray-200 bg-white">
			<!-- Header -->
			<div class="border-b border-gray-200 p-4">
				...
				<div class="mb-2 grid grid-cols-2 gap-2">
					<button
						...
						class="flex items-center justify-center space-x-1 rounded-lg bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700"
					>
					...
					<button
						...
						class="flex items-center justify-center space-x-1 rounded-lg bg-green-600 px-3 py-2 text-sm text-white hover:bg-green-700"
					>
						<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path
								stroke-linecap="round"
			...
			<div class="flex-1 overflow-y-auto p-4">
				...
				{:else if error}
					<div class="py-8 text-center text-red-600">
						<p>{error}</p>
						<button on:click={loadTreeData} class="mt-2 text-blue-600 hover:text-blue-800">
							Try Again
						</button>
					</div>
				{:else}
						...
						<nav class="flex items-center space-x-2 text-sm">
							...
							{#each navigationPath as pathNode, index}
								...
								<button
									...
									class="text-gray-600 hover:text-gray-900 hover:underline {index ===
					...
					<div class="flex items-center justify-between">
						<div>
							...
							<div class="mt-2 flex items-center space-x-4">
								<span
									class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {selectedNode.type ===
									'folder'
						...
						<div class="flex items-center space-x-3">
							{#if selectedNode.type === 'workflow'}
								...
								<div class="flex items-center space-x-2">
									<span
										class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {workflowHistory[
											selectedNode.id
										]?.source === 'GitHub Actions'
											? 'bg-green-100 text-green-800'
											: workflowHistory[selectedNode.id]?.source === 'newly-created'
												? 'bg-blue-100 text-blue-800'
												: 'bg-gray-100 text-gray-800'}"
									>
								...
								<div class="flex rounded-lg bg-gray-100 p-1">
									<button
										...
										class="rounded-md px-3 py-1 text-sm font-medium {viewMode === 'details'
									...
									<button
										...
										class="rounded-md px-3 py-1 text-sm font-medium {viewMode === 'table'
											? 'bg-blue-600 text-white shadow-sm'
											: 'text-gray-600 hover:text-gray-900'}"
									>
										📋 Build History
									...
									<button
										...
										class="rounded-md px-3 py-1 text-sm font-medium {viewMode === 'stages'
											? 'bg-blue-600 text-white shadow-sm'
											: 'text-gray-600 hover:text-gray-900'}"
									>
										🔄 Pipeline Stages
								...
								<button
									...
									class="flex items-center space-x-2 rounded-lg bg-green-600 px-4 py-2 text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50"
								>
									...
									{:else}
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
												...
												stroke-linejoin="round"
												stroke-width="2"
												d="M14.828 14.828a4 4 0 01-5.656 0M9 10h1m4 0h1m-6 4h1m4 0h1m4-2a9 9 0 11-18 0 9 9 0 0118 0z"
											/>
										</svg>
										<span>▶️ Run Now</span>
									{/if}
					...
					{#if selectedNode.type === 'folder'}
						<div class="space-y-4">
							<div class="rounded-lg bg-gray-50 p-4">
								<h3 class="mb-2 text-lg font-medium text-gray-900">📁 Folder Overview</h3>
								<p class="mb-4 text-gray-600">
									This folder contains {selectedNode.children ? selectedNode.children.length : 0} items.
								</p>
									...
									<div class="space-y-2">
										...
										<div class="grid grid-cols-1 gap-2 md:grid-cols-2">
											{#each selectedNode.children as child}
												<div class="flex items-center space-x-2 rounded border bg-white p-2">
													<span class="text-lg">
														{child.type === 'folder'
															? '📁'
															: child.type === 'workflow'
							...
							<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
								...
								<div class="border-b border-gray-200 px-6 py-4">
									<div class="flex items-center justify-between">
										...
										<div class="flex items-center space-x-2">
											<button
												on:click={() => runWorkflow(selectedNode)}
												class="inline-flex items-center rounded-md border border-transparent bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700"
												disabled={runningWorkflow === selectedNode.id}
											>
												{#if runningWorkflow === selectedNode.id}
													<div
														class="mr-2 -ml-1 h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"
													></div>
													Building...
												{:else}
													<svg
										...
										<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
											{#each Array(4).fill(0) as _, i}
												<div class="animate-pulse text-center">
													<div
														class="mx-auto mb-2 h-8 w-16 rounded bg-gray-300 text-2xl font-bold"
													></div>
													<div class="mx-auto h-4 w-20 rounded bg-gray-200 text-sm"></div>
												</div>
											{/each}
										</div>
									{:else}
										...
										<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
											...
											<div class="text-center">
												<div class="text-2xl font-bold text-green-600">
													{workflowHistory[selectedNode.id]?.runs?.filter(
														(r) => r.status === workflowStates.SUCCESS
													).length || 0}
												</div>
											...
											<div class="text-center">
												<div class="text-2xl font-bold text-red-600">
													{workflowHistory[selectedNode.id]?.runs?.filter(
														(r) => r.status === workflowStates.FAILURE
													).length || 0}
												</div>
								...
								<div class="px-6 py-4">
									{#if workflowHistory[selectedNode.id]?.loading}
										...
										<div class="space-y-4">
											<div class="flex items-center justify-between">
												<div class="h-6 w-32 animate-pulse rounded bg-gray-300"></div>
													...
													<tbody class="divide-y divide-gray-200 bg-white">
														{#each Array(5).fill(0) as _, i}
															<tr class="animate-pulse">
																...
																<td class="px-6 py-4 whitespace-nowrap">
																	<div class="flex space-x-2">
																		<div class="h-6 w-12 rounded bg-gray-300"></div>
																		<div class="h-6 w-12 rounded bg-gray-300"></div>
																	</div>
																</td>
															</tr>
									...
									{:else if workflowHistory[selectedNode.id]?.runs?.length}
										...
										<div class="overflow-hidden">
											<table class="min-w-full">
												...
												<tbody class="divide-y divide-gray-200 bg-white"
													>{#each workflowHistory[selectedNode.id].runs as run, index}
																...
																<div class="text-sm text-gray-900">
																	{#if run.commitSha}
																		<code class="rounded bg-gray-100 px-1 py-0.5 text-xs"
																			>{run.commitSha.substring(0, 7)}</code
																		>
																	{/if}
																</div>
																<div class="max-w-48 truncate text-xs text-gray-500">
									...
									{:else}
										...
										<div class="py-12 text-center">
											...
											<p class="mt-1 text-sm text-gray-500">
												Get started by running your first build.
											</p>
											<div class="mt-6">
												<button
													on:click={() => runWorkflow(selectedNode)}
								...
								<div class="mt-6 rounded-lg border border-gray-200 bg-white shadow-sm">
									...
									<div class="px-6 py-4">
										...
										{#if getWorkflowSteps(selectedNode).length > 0}
											...
											<div class="mb-4">
												<h4 class="mb-3 text-sm font-medium text-gray-900">📋 Workflow Steps</h4>
												<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
													{#each steps as step, index}
														<div class="flex items-center space-x-3 rounded-lg bg-gray-50 p-3">
															<span
																class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-medium text-blue-800"
															>
																{index + 1}
															</span>
															<div class="flex-1">
										...
										<!-- YAML Content -->
										<div class="max-h-64 overflow-x-auto rounded-lg bg-gray-900 p-4">
											<pre
												class="font-mono text-sm whitespace-pre-wrap text-green-400">{selectedNode.content}</pre>
										</div>
									</div>
								</div>
							{/if}
										...
										<div class="space-y-6">
											...
											>
												...
												<div class="font-medium text-blue-700">
													{workflowHistory[selectedNode.id].loadingMessage ||
														'Fetching workflow history from GitHub Actions...'}
												</div>
											...
											<div class="grid grid-cols-1 gap-4 md:grid-cols-4">
												{#each Array(4).fill(0) as _, i}
													<div
														class="animate-pulse rounded-lg border border-gray-200 bg-gray-50 p-3"
													>
														<div class="mb-2 h-4 w-20 rounded bg-gray-300"></div>
														<div class="h-8 w-12 rounded bg-gray-300"></div>
													</div>
												{/each}
											</div>
											...
											<div class="space-y-4">
												<div class="animate-pulse">
													<div class="mb-2 h-6 w-1/3 rounded bg-gray-200"></div>
												...
												>
													{#each Array(8).fill(0) as _, i}
														<div
															class="animate-pulse rounded-lg border border-gray-200 bg-gray-50 p-4"
														>
															<!-- Skeleton Header -->
															<div class="mb-3 flex items-center justify-between">
																<div class="flex items-center space-x-2">
																	<div class="h-4 w-8 rounded-full bg-gray-300"></div>
																	<div class="h-6 w-6 rounded bg-gray-300"></div>
																</div>
																<div class="h-4 w-16 rounded-full bg-gray-300"></div>
															</div>
												...
												<div class="mt-4 animate-pulse rounded-lg bg-gray-100 p-4">
													<div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
														{#each Array(4).fill(0) as _}
															<div class="text-center">
																<div class="mx-auto mb-2 h-6 w-8 rounded bg-gray-300"></div>
																<div class="mx-auto h-4 w-16 rounded bg-gray-300"></div>
															</div>
														{/each}
													</div>
												</div>
									...
									{:else if workflowHistory[selectedNode.id]?.runs?.length}
										...
										<div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
											...
											<div class="rounded-lg border border-green-200 bg-green-50 p-3">
												...
												<div class="text-2xl font-bold text-green-600">
													{Math.round(
														(runs.filter((r) => r.status === workflowStates.SUCCESS).length /
															runs.length) *
															100
													)}%
												</div>
											...
											<div class="rounded-lg border border-yellow-200 bg-yellow-50 p-3">
												...
												<div class="text-2xl font-bold text-yellow-600">
													{workflowHistory[selectedNode.id].avgDuration
														? formatDuration(workflowHistory[selectedNode.id].avgDuration)
														: 'N/A'}
												</div>
											</div>
											<div class="rounded-lg border border-purple-200 bg-purple-50 p-3">
												...
												<div class="flex items-center text-xl font-bold text-purple-600">
													{getStatusIcon(runs[0]?.status)}
										...
										<div class="mb-6">
											...
											<div class="overflow-x-auto">
												<div class="min-w-full">
													<!-- Header Row -->
													<div
														class="grid auto-cols-max grid-flow-col gap-6 border border-gray-300 bg-gray-50 p-3"
													>
														<div class="min-w-32 text-center text-sm font-medium text-gray-700">
															Run #
														</div>
														{#each workflowSteps as step, stepIndex}
															<div
																class="min-w-28 text-center text-sm font-medium text-gray-700"
																title={step.name}
															>
														...
														{/each}
														<div class="min-w-24 text-center text-sm font-medium text-gray-700">
															Duration
														</div>
														<div class="min-w-28 text-center text-sm font-medium text-gray-700">
															Status
														</div>
													</div>
													...
													{#each runs.slice(0, 10) as run, runIndex}
														<div
															class="grid auto-cols-max grid-flow-col gap-6 border-r border-b border-l border-gray-300 p-3 transition-colors duration-150 {run.status ===
															workflowStates.SUCCESS
																? 'bg-green-25 hover:bg-green-50'
																: run.status === workflowStates.FAILURE
														...
														>
															...
															{#each workflowSteps as step, stepIndex}
																{@const runStep =
																	run.steps?.find(
																		(s) => s.id === step.id || s.name === step.name
																	) || run.steps?.[stepIndex]}
										...
										{#if runs.length > 0}
											...
											<div class="mb-6">
													...
													<div class="grid grid-cols-1 gap-4 text-sm md:grid-cols-3">
														<div>
															...
															<span class="ml-2 text-gray-600"
																>{latestRun.commitSha?.substring(0, 8) || 'N/A'}</span
															>
														</div>
														<div>
															<span class="font-medium text-gray-700">Author:</span>
															<span class="ml-2 text-gray-600">{latestRun.author || 'Unknown'}</span
															>
														</div>
														<div>
															<span class="font-medium text-gray-700">Branch:</span>
															<span class="ml-2 text-gray-600">{latestRun.branch || 'main'}</span>
														</div>
														{#if latestRun.commitMessage}
															<div class="col-span-full">
																<span class="font-medium text-gray-700">Message:</span>
																<span class="ml-2 text-gray-600">{latestRun.commitMessage}</span>
															</div>
														{/if}
													</div>
												</div>
												...
												{#if latestRun.steps?.length}
													<div class="space-y-4">
														<div class="flex items-center justify-between">
															<h5 class="flex items-center text-lg font-medium text-gray-900">
																🔄 Step Execution Grid
																<span class="ml-2 text-sm text-gray-500"
																	>({latestRun.steps.length} steps)</span
																>
															</h5>
														...
														<div class="responsive-step-grid step-execution-grid">
															{#each latestRun.steps as step, index}
																<div
																	class="group step-card relative rounded-lg border p-4 transition-all duration-200 hover:shadow-md {step.status ===
																	workflowStates.SUCCESS
																		? 'border-green-300 bg-green-50 hover:bg-green-100'
																		: step.status === workflowStates.FAILURE
																			? 'border-red-300 bg-red-50 hover:bg-red-100'
																			: step.status === workflowStates.RUNNING
																...
																>
																	...
																	<div class="mb-3 flex items-center justify-between">
																		<div class="flex items-center space-x-2">
																			<span
																				class="rounded-full bg-white px-2 py-1 text-xs font-medium text-gray-700 shadow-sm"
																			>
																				#{index + 1}
																			</span>
																			<span
																				class="text-lg"
																				title={step.status?.toUpperCase() || 'PENDING'}
																				>{getStatusIcon(step.status)}</span
																			>
																		</div>
																		<span
																			class="rounded-full px-2 py-1 text-xs font-medium {step.status ===
																			workflowStates.SUCCESS
																	...
																	<!-- Step Title -->
																	<h6
																		class="step-title mb-3 text-sm leading-tight font-medium text-gray-900"
																	>
																		{step.name}
																	</h6>
																	...
																	<div class="space-y-2 text-xs">
																		...
																		<div class="flex items-center justify-between py-1">
																			...
																			<span class="font-medium text-gray-900"
																				>{formatDuration(step.duration || 0)}</span
																			>
																		</div>
																		...
																		{#if step.startTime}
																			<div class="flex items-center justify-between py-1">
																				...
																				<span class="text-gray-800"
																					>{formatRelativeTime(step.startTime)}</span
																				>
																			</div>
																		{/if}
																		...
																		{#if step.jobName && step.jobName !== 'unknown'}
																			<div class="py-1">
																				...
																				<span class="ml-1 text-gray-800">{step.jobName}</span>
																			</div>
																		{/if}
																		...
																		{#if step.action}
																			<div class="mt-3 border-t border-gray-200 pt-2">
																				<span class="mb-1 block text-gray-600">Action:</span>
																				<code
																					class="step-action-code block rounded border bg-white px-2 py-1 text-xs leading-relaxed break-all"
																				>
																					{step.action.length > 60
																						? step.action.substring(0, 60) + '...'
																						: step.action}
																				</code>
																			</div>
																		{/if}
															...
															<div class="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
																<div class="text-center">
																	<div class="text-lg font-bold text-green-600">
																		{latestRun.steps.filter(
																			(s) => s.status === workflowStates.SUCCESS
																		).length}
																	</div>
																...
																<div class="text-center">
																	<div class="text-lg font-bold text-red-600">
																		{latestRun.steps.filter(
																			(s) => s.status === workflowStates.FAILURE
																		).length}
																	</div>
																...
																<div class="text-center">
																	<div class="text-lg font-bold text-blue-600">
																		{latestRun.steps.filter(
																			(s) => s.status === workflowStates.RUNNING
																		).length}
																	</div>
																...
																<div class="text-center">
																	<div class="text-lg font-bold text-gray-600">
																		{latestRun.steps.filter(
																			(s) => !s.status || s.status === workflowStates.PENDING
																		).length}
																	</div>
										...
										<!-- Actions -->
										<div class="mt-6 flex space-x-4 border-t border-gray-200 pt-4">
											{#if workflowHistory[selectedNode.id].repositoryUrl}
												<a
													href="{workflowHistory[selectedNode.id].repositoryUrl}/actions"
													target="_blank"
													class="inline-flex items-center rounded-md border border-transparent bg-blue-100 px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-200"
												>
													<svg
														class="mr-2 h-4 w-4"
														fill="none"
													...
													>
														<path
															...
															stroke-linejoin="round"
															stroke-width="2"
															d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-2M7 7h10v10M17 7l-4 4"
														/>
													</svg>
													View on GitHub
												</a>
										...
										<div class="rounded-lg border border-red-200 bg-red-50 p-6">
											<div class="flex items-start space-x-4">
												...
												<div class="flex-1">
													...
													<div class="mt-4 rounded-lg border border-red-200 bg-red-100 p-3">
													...
													<div class="mt-6 flex flex-wrap gap-3">
														<button
															on:click={() => loadWorkflowHistory(selectedNode)}
															class="inline-flex items-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-red-700"
														>
															<svg
																class="mr-2 h-4 w-4"
																fill="none"
														...
														{#if selectedNode.repositoryUrl}
															<a
																...
																target="_blank"
																class="inline-flex items-center rounded-md border border-red-300 bg-white px-4 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-50"
															>
																<svg
																	class="mr-2 h-4 w-4"
																	fill="none"
							...
							<div class="bg-white">
													...
													<tr class="hover:bg-gray-50">
														<td class="px-6 py-4 whitespace-nowrap">
															<div class="flex items-center">
																...
																{#if run.commitSha}
																	<div class="ml-2 font-mono text-xs text-gray-400">
																		{run.commitSha}
																	</div>
																{/if}
															</div>
								...
								{:else}
									<div class="py-8 text-center">
										...
										<p class="mt-1 text-sm text-gray-500">
											Run your workflow to see build history.
										</p>
										<div class="mt-6">
											<button
												on:click={() => runWorkflow(selectedNode)}
			...
			{:else}
				...
				<div class="flex flex-1 items-center justify-center">
					<div class="text-center">
						...
						<h3 class="mb-2 text-lg font-medium text-gray-900">Select an item to view details</h3>
						<p class="text-gray-600">
							Click on any folder or workflow in the tree to see its contents and metadata.
						</p>
					</div>
				</div>
			{/if}
...
{#if showContextMenu}
	<div
		class="context-menu fixed z-50 rounded-lg border border-gray-200 bg-white py-2 shadow-lg"
	...
	<div class="bg-opacity-50 fixed inset-0 z-50 h-full w-full overflow-y-auto bg-gray-600">
		<div class="relative top-20 mx-auto w-96 rounded-md border bg-white p-5 shadow-lg">
			<div class="mt-3">
				...
				<div class="mb-4">
					<label class="mb-2 block text-sm font-medium text-gray-700" for="new-item-name">
...
{#if showFileEditor}
	<div class="bg-opacity-50 fixed inset-0 z-50 h-full w-full overflow-y-auto bg-gray-600">
		<div class="relative top-10 mx-auto w-11/12 max-w-4xl rounded-md border bg-white p-5 shadow-lg">
			<div class="mb-4 flex items-center justify-between">
			...
			<div class="mb-4">
				<textarea
					...
					class="h-96 w-full rounded-md border border-gray-300 px-3 py-2 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
...
{#if showBuildParameters}
	<div class="bg-opacity-50 fixed inset-0 z-50 h-full w-full overflow-y-auto bg-gray-600">
		<div class="relative top-10 mx-auto w-11/12 max-w-3xl rounded-md border bg-white p-5 shadow-lg">
			<div class="mb-4 flex items-center justify-between">
				<div>
					...
					<p class="mt-1 text-sm text-gray-600">
						Configure and run: <strong>{selectedNode?.name || 'Unknown Workflow'}</strong>
					</p>
				</div>
				<button
			...
			<div class="mb-4 max-h-96 overflow-y-auto">
				<div class="space-y-4">
					{#each Object.entries(buildParameters) as [key, param]}
						<div class="rounded-r border-l-4 border-blue-400 bg-gray-50 py-2 pl-4">
							<label for="param-{key}" class="mb-1 block text-sm font-medium text-gray-900">
								{key.replace(/_/g, ' ')}
							...
							{#if param.type === 'boolean'}
								<label class="flex cursor-pointer items-center space-x-2">
									...
									<span class="text-sm text-gray-700"
										>Enable {key.replace(/_/g, ' ').toLowerCase()}</span
									>
								</label>
...
{#if saveStatus}
	<div class="fixed right-4 bottom-4 z-50">
		<div
			class="rounded-lg border bg-white p-4 shadow-lg {saveSuccess
				? 'border-green-200'
				: 'border-red-200'}"
		>
			<div class="flex items-center">
				...
				<div class="ml-3">
					<p class="text-sm font-medium {saveSuccess ? 'text-green-800' : 'text-red-800'}">
						{saveStatus}
					</p>
				</div>
			</div>
