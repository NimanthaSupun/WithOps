				...
				{:else if summary}
					...
					<header class="view-header">
						...
						<div class="header-cta">
							<div
								class="score-pill"
								style="background:{classColors[summary.classification]
									?.bg}; border-color:{classColors[summary.classification]?.color}"
							>
								<span class="score-lbl">PERFORMANCE</span>
								<span class="score-num" style="color:{classColors[summary.classification]?.color}"
									>{classColors[summary.classification]?.label || 'N/A'}</span
								>
							</div>
								...
								<div class="stat-card">
									<div class="stat-icon" style="background:{cls.bg};color:{cls.color}">
										...
										{:else if i === 2}<svg
												...
												stroke-width="2"
												><path
													d="M12 9v2m0 4h.01M5.07 19h13.86c1.54 0 2.5-1.67 1.73-2.5L13.73 4c-.77-1.33-2.69-1.33-3.46 0L3.34 16.5C2.57 17.33 3.53 19 5.07 19z"
												/></svg
											>
										{:else}<svg
												fill="none"
												...
												stroke-width="2"
												><path
													d="M4 4v5h.58m15.36 2A8 8 0 004.58 9m0 0H9m11 11v-5h-.58m0 0a8 8 0 01-15.36-2m15.36 2H15"
												/></svg
											>
										{/if}
									</div>
						...
						<div class="intel-card">
							...
							<div class="benchmark-table">
								<table>
									...
									<tbody>
										<tr
											><td>Deploy Frequency</td><td>Multiple/day</td><td>Weekly</td><td>Monthly</td
											><td>&lt; Monthly</td></tr
										>
										<tr
											><td>Lead Time</td><td>&lt; 1 hour</td><td>&lt; 1 day</td><td>&lt; 1 week</td
											><td>&gt; 1 week</td></tr
										>
										<tr
											><td>Change Failure Rate</td><td>0-5%</td><td>5-15%</td><td>15-30%</td><td
												>&gt; 30%</td
											></tr
										>
										<tr
											><td>MTTR</td><td>&lt; 1 hour</td><td>&lt; 1 day</td><td>&lt; 1 week</td><td
												>&gt; 1 week</td
											></tr
										>
									</tbody>
						...
						<div class="intel-card">
							...
							<div class="deploy-summary">
								<div class="deploy-stat">
									<span class="deploy-label">Total Deployments</span><span class="deploy-val"
										>{summary.total_deployments || 0}</span
									>
								</div>
								<div class="deploy-stat">
									<span class="deploy-label">Period</span><span class="deploy-val"
										>{summary.period_days} days</span
									>
								</div>
							</div>
						</div>
					...
					{#if activeTab === 'repos'}
						<div class="intel-card">
							...
							{#if repoData?.repositories?.length > 0}
								<div class="repo-table-wrap">
									<table class="repo-table">
										<thead
											><tr
												><th>Repository</th><th>Deploy Freq</th><th>Lead Time</th><th
													>Failure Rate</th
												><th>MTTR</th><th>Level</th><th>Deploys</th></tr
											></thead
										>
										<tbody>
											{#each repoData.repositories as repo}
												...
												<tr>
													...
													<td
														><span class="level-badge" style="background:{cls.bg};color:{cls.color}"
															>{cls.label}</span
														></td
													>
							...
							{:else}
								<div class="empty-msg">
									No deployment data found. Connect repositories and trigger CI/CD pipelines to
									start tracking.
								</div>
							{/if}
						</div>
					...
					{#if activeTab === 'trends'}
						<div class="intel-card">
										...
										<div class="trend-row">
											<div class="trend-period">
												...
												<span class="trend-date"
													>{new Date(point.period_end).toLocaleDateString('en-US', {
														month: 'short',
														day: 'numeric'
													})}</span
												>
											...
											<div class="trend-metrics">
												<span class="trend-metric" title="Deploy Freq"
													>{point.deployment_frequency.toFixed(2)}/d</span
												>
												<span class="trend-metric" title="Lead Time"
													>{formatDuration(point.lead_time_seconds)}</span
												>
												<span class="trend-metric" title="Failure Rate"
													>{(point.change_failure_rate * 100).toFixed(0)}%</span
												>
												<span class="trend-metric" title="MTTR"
													>{formatDuration(point.mttr_seconds)}</span
												>
											</div>
											<span class="level-badge" style="background:{cls.bg};color:{cls.color}"
												>{cls.label}</span
							...
							{:else}
								<div class="empty-msg">
									No trend data available yet. Deployment events will populate trends over time.
								</div>
							{/if}
						</div>
					{/if}
					...
					{#if activeTab === 'correlation'}
						<div class="intel-card">
							...
							<p class="card-subtitle">
								Measuring the relationship between security maturity and delivery performance
							</p>
								...
								<div class="correlation-grid">
									<div class="corr-card">
										...
										<div class="corr-value">
											{correlationData.dsomm_data?.average_maturity_score?.toFixed(1) || 'N/A'}<span
												class="corr-unit">/100</span
											>
										</div>
										<div class="corr-detail">
											{correlationData.dsomm_data?.total_assessments || 0} assessments
										</div>
									</div>
									<div class="corr-card">
										...
										<div
											class="corr-value"
											style="color:{(
												classColors[correlationData.dora_metrics?.classification] || classColors.low
											).color}"
										>
											{(
												classColors[correlationData.dora_metrics?.classification] || classColors.low
										...
										<div class="corr-detail">
											{correlationData.dora_metrics?.total_deployments || 0} deployments
										</div>
									</div>
								</div>
							...
							{:else}
								<div class="empty-msg">
									Run a DSOMM maturity assessment from the <a
										href="/github/workspace/{org}/intelligence">Intelligence</a
									> page to enable correlation analysis.
								</div>
							{/if}
						</div>
					{/if}
