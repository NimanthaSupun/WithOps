<script>
	...
	onMount(async () => {
		...
		console.log(
			`🎨 Canvas parameters - Org: ${orgName}, Repo: ${selectedRepo}, Workflow: ${selectedWorkflowName}`
		);
	...
	$: if (workflowConnections && Array.isArray(workflowConnections)) {
		...
		connectionUpdateTimeout = setTimeout(() => {
			if (typeof window !== 'undefined') {
				...
				connectionPaths = workflowConnections
	...
	function processWorkflowsForCanvas() {
		...
		if (selectedRepo) {
			filteredWorkflows = filteredWorkflows.filter(
				(workflow) => workflow.repository === selectedRepo
			);
			console.log(
				`🎯 Filtered by repository '${selectedRepo}':`,
		...
		if (selectedWorkflowName) {
			const targetWorkflow = filteredWorkflows.find(
			...
			if (targetWorkflow) {
				...
				console.log(
					`🎯 Filtered by workflow '${selectedWorkflowName}' with connections:`,
					filteredWorkflows.length,
					'workflows'
				);
				console.log(
					`📋 Final workflow list:`,
					filteredWorkflows.map((w) => w.name)
				);
			} else {
				console.warn(
					`⚠️ Workflow '${selectedWorkflowName}' not found in repository '${selectedRepo}'`
				);
				filteredWorkflows = [];
			}
		}
		...
		console.log(
			'📊 Workflow data keys for first workflow:',
			workflows[0] ? Object.keys(workflows[0].originalData) : 'No workflows'
		);
		...
		allWorkflows.forEach((workflow) => {
			...
			if (workflow.uses && Array.isArray(workflow.uses)) {
				workflow.uses.forEach((use) => {
					if (
						use.includes(targetWorkflowName) ||
...
jobs:
  call-reusable:
      ...
      node-version: '18'`;
				...
				if (content) {
					...
					if (
						...
						isConnected = true;
						console.log(
							`📄 Content-based connection found: ${direction === 'outgoing' ? targetWorkflowName + ' -> ' + workflow.name : workflow.name + ' -> ' + targetWorkflowName}`
						);
					}
				}
			});
		...
		console.log(
			`🔗 Found ${connectedWorkflows.length} bidirectionally connected workflows for '${targetWorkflowName}':`,
	...
	function detectWorkflowConnections() {
		...
		console.log(
			'📋 Available workflows:',
			workflows.map((w) => ({ id: w.id, name: w.name, path: w.path }))
		);
		...
		workflows.forEach((workflow, workflowIndex) => {
			...
			console.log(
				`🔍 [${workflowIndex + 1}/${workflows.length}] Checking workflow: ${workflow.name} (${workflow.path})`
			);
  ...
  call-reusable:
    ...
    secrets:
      CUSTOM_TOKEN: \${{ secrets.MY_CUSTOM_TOKEN }}`;
						...
						if (usesValue.includes('./.github/workflows/')) {
							...
							if (targetWorkflow) {
								...
								if (!existingConnection) {
									...
									console.log(
										`✅ Added new connection: ${workflow.name} -> ${targetWorkflow.name}`
									);
								} else {
					...
					if (
						...
						for (let j = i + 1; j < Math.min(i + 15, lines.length); j++) {
							...
							if (nextTrimmed.startsWith('uses:') && nextTrimmed.includes('./.github/workflows/')) {
								...
								if (targetWorkflow) {
									console.log(
										`✅ Found job-level connection: ${workflow.name} -> ${targetWorkflow.name}`
									);
									...
									if (!existingConnection) {
										...
										console.log(
											`✅ Added job-level connection: ${workflow.name} -> ${targetWorkflow.name}`
										);
									} else {
				...
				Object.entries(workflowData.jobs).forEach(([jobName, job]) => {
					if (job && typeof job === 'object' && job.uses && typeof job.uses === 'string') {
						...
						if (job.uses.includes('./.github/workflows/')) {
							...
							if (targetWorkflow) {
								console.log(
									`✅ Found parsed job connection: ${workflow.name} -> ${targetWorkflow.name}`
								);
								// Check for duplicates
								const existingConnection = connections.find(
									(c) =>
								...
								if (!existingConnection) {
									...
									console.log(
										`✅ Added parsed job connection: ${workflow.name} -> ${targetWorkflow.name}`
									);
								} else {
		...
		const uniqueConnections = connections.filter((connection, index, self) => {
			...
			if (!isFirst) {
				console.log(
					`🔄 Removing duplicate connection: ${connection.fromName} -> ${connection.toName}`
				);
			}
		...
		console.log(
			`🎯 Final connections:`,
			uniqueConnections.map((c) => `${c.fromName} -> ${c.toName}`)
		);
		...
		try {
			...
			if (!workflowContent) {
				...
				if (workflowData.content && typeof workflowData.content === 'string') {
					workflowContent = workflowData.content;
					console.log(
						`📝 Using workflow content from originalData (${workflowContent.length} chars)`
					);
				} else if (workflowData.yaml_content && typeof workflowData.yaml_content === 'string') {
					workflowContent = workflowData.yaml_content;
					console.log(
						`📝 Using workflow yaml_content from originalData (${workflowContent.length} chars)`
					);
				} else if (workflowData.file_content && typeof workflowData.file_content === 'string') {
					workflowContent = workflowData.file_content;
					console.log(
						`📝 Using workflow file_content from originalData (${workflowContent.length} chars)`
					);
				}
			}
			...
			if (workflowContent && workflowContent.length > 0) {
				...
				console.log(
					`✅ Parsed ${selectedWorkflowSteps.length} workflow steps for ${workflow.name}:`,
					selectedWorkflowSteps
				);
			} else {
	...
	function parseWorkflowSteps(yamlContent, workflow) {
		...
		try {
			...
			if (steps.length === 0) {
				console.log(
					'🔍 No steps found in standard format, checking for workflow_call or other formats...'
				);
		...
		console.log(
			'📋 Steps details:',
			steps.map((s) => ({ name: s.name, type: s.type, detail: s.detail }))
		);
		return steps;
	}
	...
	function addActionToWorkflow(predefinedAction, insertAtPosition = -1) {
		...
		const positionText =
			insertAtPosition === 0
				? 'beginning'
				: insertAtPosition === selectedWorkflowSteps.length - 1
					? 'end'
		...
		console.log(
			`✅ Added action "${predefinedAction.name}" to workflow "${selectedWorkflow.name}" at position ${insertAtPosition + 1}`
		);
	}
		...
		try {
			if (draggedItem && selectedWorkflowId && showWorkflowSteps) {
				...
				const insertPosition =
					dropZonePosition !== -1 ? dropZonePosition : selectedWorkflowSteps.length;
...
<svelte:head>
	<title
		>Canvas Workflow Builder{selectedWorkflowName
			? ` - ${selectedWorkflowName}`
			: selectedRepo
				? ` - ${selectedRepo}`
				: ''} - {orgName} - WithOps</title
	>
</svelte:head>

<div class="canvas-container {darkMode ? 'dark' : 'light'}">
	<!-- Top Navigation Bar -->
			...
			<div class="navbar-left">
				<div class="brand-section">
					...
					<div class="brand-text">
						...
						<span class="brand-subtitle">Canvas Builder</span>
					</div>
				</div>
				...
				<nav class="breadcrumb">
					...
					<span class="breadcrumb-separator">/</span>
					<span class="breadcrumb-current">Canvas</span>
				</nav>
			</div>
	...
	<div class="main-content">
		...
		<div class="content-header">
			<div class="header-left">
				<h1 class="page-title">🎨 Canvas Workflow Builder</h1>
				<div class="page-info">
					{#if selectedWorkflowName && selectedRepo}
						<p class="page-subtitle">
							Editing workflow: <span class="highlight">{selectedWorkflowName}</span> from
							<span class="highlight">{selectedRepo}</span>
						</p>
					{:else if selectedRepo}
						<p class="page-subtitle">
							Repository workflows: <span class="highlight">{selectedRepo}</span>
						</p>
					{:else}
						<p class="page-subtitle">
							All workflows in organization: <span class="highlight">{orgName}</span>
						</p>
					{/if}
					{#if workflows.length > 0}
			...
			<div class="header-actions">
				...
				{#if selectedWorkflow}
					<button onclick={saveWorkflowChanges} disabled={isSaving} class="save-button">
						{#if isSaving}
		...
		{#if saveMessage}
			<div class="status-message">
				<div class="status-card {saveSuccess ? 'success' : 'error'}">
					...
					<p class="status-text">{saveMessage}</p>
				</div>
			</div>
		{/if}
		...
		{:else if workflows.length === 0}
			...
			<div class="rounded-lg bg-white p-12 shadow-lg">
				<div class="text-center">
					...
					{#if selectedWorkflowName}
						<h3 class="mb-2 text-lg font-medium text-gray-900">Workflow Not Found</h3>
					...
					{:else if selectedRepo}
						<h3 class="mb-2 text-lg font-medium text-gray-900">No Workflows Found</h3>
						<p class="mb-6 text-sm text-gray-600">
							The repository "<span class="font-medium">{selectedRepo}</span>" doesn't have any
							GitHub Actions workflows yet.
						</p>
					{:else}
						<h3 class="mb-2 text-lg font-medium text-gray-900">No Workflows Available</h3>
						<p class="mb-6 text-sm text-gray-600">
					...
					<div class="flex justify-center space-x-3">
						...
						{#if selectedRepo}
							<button
								onclick={() => goto(`/github/workspace/${orgName}/canvas`)}
		...
		{:else}
			...
			<div class="overflow-hidden rounded-lg bg-white shadow-lg">
				<div class="flex" style="height: calc(100vh - 200px)">
					...
					{#if showAddActionPanel}
						<div class="w-64 overflow-y-auto border-r border-gray-200 bg-gray-50 p-4">
							<div class="mb-4 flex items-center justify-between">
								...
								<button
									onclick={() => (showAddActionPanel = false)}
							...
							<p class="mb-4 text-sm text-gray-600">
								Drag actions to add them to the selected workflow
							</p>
							...
							<div class="space-y-2">
								{#each predefinedActions as action}
									<div
										...
										class="cursor-move rounded-lg border border-gray-200 bg-white p-3 transition-all duration-200 hover:border-blue-300 hover:shadow-sm"
										role="button"
										tabindex="0"
										aria-label={`Drag ${action.name} action to add to workflow`}
										onkeydown={(e) => e.key === 'Enter' && handleDragStart(e, action)}
									>
										<div class="text-sm font-medium text-gray-900">{action.name}</div>
										<div class="mt-1 text-xs text-gray-500">{action.description}</div>
					...
					{:else}
						<!-- Collapsed Add Panel -->
						<div class="flex w-12 flex-col items-center border-r border-gray-200 bg-gray-50 pt-4">
							<button
								onclick={() => (showAddActionPanel = true)}
								class="mb-4 text-gray-400 hover:text-gray-600"
					...
					<div class="flex flex-1 flex-col">
						...
						<!-- Canvas Area -->
						<div
							class="canvas-area relative flex-1 overflow-auto bg-gray-100"
						...
						>
							...
							{#each workflows as workflow}
								<div
									id={workflow.id}
									class="absolute cursor-move rounded-lg border-2 bg-white shadow-lg transition-all duration-200 {selectedWorkflowId ===
									workflow.id
										? 'border-blue-500 shadow-xl'
										: 'border-gray-200 hover:border-gray-300'}"
									style="left: {workflow.x}px; top: {workflow.y}px; width: 280px; user-select: none;"
									onmousedown={(e) => handleWorkflowMouseDown(e, workflow)}
									onclick={(e) => selectWorkflow(workflow.id, e)}
									role="button"
									tabindex="0"
									onkeydown={(e) => e.key === 'Enter' && selectWorkflow(workflow.id, e)}
								>
									<div class="p-4">
										...
										<div class="mb-2 text-xs text-gray-600">📁 {workflow.repository}</div>
										<div class="mb-2 truncate font-mono text-xs text-gray-500">{workflow.path}</div>

										<div class="text-xs text-gray-400 italic">Click to view workflow steps</div>
									</div>
								</div>
								...
								{#if selectedWorkflowId === workflow.id && showWorkflowSteps && selectedWorkflowSteps.length > 0}
									<div
										class="absolute rounded-lg border-2 border-blue-300 bg-gray-50 p-4 shadow-xl"
										style="left: {workflow.x}px; top: {workflow.y +
											160}px; width: 280px; max-height: 300px; overflow-y: auto; z-index: 5;"
										ondragover={handleDragOver}
										ondrop={handleDrop}
										role="application"
									...
									>
										<div class="mb-3 flex items-center justify-between">
											...
											<button
												onclick={() => (showWorkflowSteps = false)}
												class="text-gray-400 hover:text-gray-600"
										...
										<div class="space-y-1">
											...
											{#if showDropZones && isDragging}
												<div
													class="flex h-8 items-center justify-center rounded border-2 border-dashed border-blue-400 bg-blue-50 text-xs text-blue-600 transition-all duration-200 {dropZonePosition ===
													0
											...
											{#each selectedWorkflowSteps as step, index}
												<div
													class="cursor-pointer rounded border bg-white p-2 transition-colors duration-200 hover:border-blue-300 {selectedStepAction?.id ===
													step.id
														? 'border-blue-500 bg-blue-50'
														: 'border-gray-200'}"
													draggable="true"
													ondragstart={(e) => handleStepDragStart(e, step, index)}
													ondragend={handleStepDragEnd}
													onclick={() => selectStepAction(step)}
													role="button"
													tabindex="0"
													onkeydown={(e) => e.key === 'Enter' && selectStepAction(step)}
												>
													<div class="mb-1 flex items-center space-x-1">
														<span
															class="inline-flex h-4 w-4 items-center justify-center rounded-full bg-blue-100 text-xs font-medium text-blue-800"
														>
															{index + 1}
														</span>
														{#if step.isNew}
													...
													<div class="truncate text-xs font-medium text-gray-900" title={step.name}>
														{step.name}
													</div>
													<div class="truncate text-xs text-gray-500" title={step.detail}>
												...
												{#if showDropZones && (isDragging || draggedStep)}
													<div
														class="flex h-6 items-center justify-center rounded border-2 border-dashed border-blue-400 bg-blue-50 text-xs text-blue-600 transition-all duration-200 {dropZonePosition ===
															index + 1 || stepDropZonePosition === index + 1
															? 'border-blue-600 bg-blue-100'
															: ''}"
														ondragenter={() => {
										...
										<div class="mt-3 text-center text-xs text-gray-500">
											{#if showDropZones && (isDragging || draggedStep)}
												<div class="rounded border border-blue-200 bg-blue-50 px-2 py-1">
													🎯 Drop {draggedStep ? 'step' : 'action'} at the blue zones to insert at specific
													positions
												</div>
											{:else}
												<div class="space-y-1">
													<div>
														Drag actions from the left panel to add new steps at any position
													</div>
							...
							{#if workflowConnections.length > 0 && connectionPaths && connectionPaths.length > 0}
								...
								<!-- Connection Legend -->
								<div class="absolute top-4 right-4 z-10 rounded-lg border bg-white p-3 shadow-lg">
									<h5 class="mb-2 text-sm font-semibold text-gray-900">🔗 Workflow Connections</h5>
							...
							{:else if workflowConnections.length > 0}
								...
								<div
									class="absolute top-4 right-4 z-10 max-w-md rounded-lg border border-yellow-200 bg-yellow-50 p-3"
								>
									...
									<div class="text-xs text-yellow-700">
										...
										<div class="mt-2 max-h-32 overflow-y-auto">
											{#each workflowConnections as conn, index}
												<div class="mb-2 rounded bg-yellow-100 p-2">
													<div>• {conn.fromName || 'Unknown'} → {conn.toName || 'Unknown'}</div>
													<div class="ml-4 text-xs text-yellow-600">
														From: {typeof conn.from} - {JSON.stringify(conn.from)}
													</div>
													<div class="ml-4 text-xs text-yellow-600">
														To: {typeof conn.to} - {JSON.stringify(conn.to)}
													</div>
												</div>
											{/each}
										</div>
							...
							{#if isDragging && selectedWorkflowId && showWorkflowSteps}
								<div
									class="bg-opacity-50 pointer-events-none absolute inset-0 flex items-center justify-center border-2 border-dashed border-blue-400 bg-blue-100"
									style="z-index: 10;"
									role="alert"
									aria-live="polite"
								>
									<div class="px-4 text-center text-lg font-semibold text-blue-700">
										Drop action here to add to "{workflows.find((w) => w.id === selectedWorkflowId)
											?.name || 'selected workflow'}" steps
									</div>
								</div>
							{:else if isDragging && selectedWorkflowId && !showWorkflowSteps}
								<div
									class="bg-opacity-50 pointer-events-none absolute inset-0 flex items-center justify-center border-2 border-dashed border-yellow-400 bg-yellow-100"
									style="z-index: 10;"
									role="alert"
									aria-live="polite"
								>
									<div class="px-4 text-center text-lg font-semibold text-yellow-700">
										Click on "{workflows.find((w) => w.id === selectedWorkflowId)?.name ||
											'selected workflow'}" to open steps first
									</div>
								</div>
							{:else if isDragging && !selectedWorkflowId}
								<div
									class="bg-opacity-50 pointer-events-none absolute inset-0 flex items-center justify-center border-2 border-dashed border-red-400 bg-red-100"
									style="z-index: 10;"
									role="alert"
									aria-live="polite"
								>
									<div class="px-4 text-center text-lg font-semibold text-red-700">
					...
					{#if showActionDetails && selectedStepAction}
						<div class="flex w-80 flex-col border-l border-gray-200 bg-white">
							...
							<div class="border-b border-gray-200 p-4">
								<div class="flex items-center justify-between">
									<div>
										...
										<p class="mt-1 text-xs text-gray-500">Step {selectedStepAction.step + 1}</p>
									</div>
							...
							<div class="flex-1 overflow-y-auto p-4">
								<div class="space-y-4">
									<div>
										<div class="mb-1 block text-sm font-medium text-gray-700">Name</div>
										<div class="rounded border bg-gray-50 p-2 text-sm text-gray-900">
											{selectedStepAction.name}
										</div>
									</div>

									<div>
										<div class="mb-1 block text-sm font-medium text-gray-700">Type</div>
										<div class="rounded border bg-gray-50 p-2 text-sm text-gray-900 capitalize">
											{selectedStepAction.type}
										</div>
									</div>
									...
									<div>
										...
										<div class="rounded border bg-gray-50 p-2 font-mono text-sm text-gray-900">
										...
										<div>
											...
											<div class="rounded border bg-gray-50 p-2">
												{#each Object.entries(selectedStepAction.with) as [key, value]}
													<div class="mb-1 font-mono text-sm text-gray-900">
									...
									{#if selectedStepAction.job}
										<div>
											<div class="mb-1 block text-sm font-medium text-gray-700">Job</div>
											<div class="rounded border bg-gray-50 p-2 text-sm text-gray-900">
												{selectedStepAction.job}
											</div>
										</div>
									{/if}

									<div>
										<div class="mb-1 block text-sm font-medium text-gray-700">Step Position</div>
										<div class="rounded border bg-gray-50 p-2 text-sm text-gray-900">
											{selectedStepAction.step + 1}
										</div>
									</div>
											...
											<div class="flex">
												...
												<div class="ml-3">
													...
													<p class="mt-1 text-sm text-green-700">
														This step will be added when you save changes.
													</p>
												</div>
											</div>
										</div>
...
<style>
	...
	.status-card.success .icon-svg {
		color: #00ff88;
	}

	.status-card.error .icon-svg {
		color: #ff5757;
	}
