...
<div class="workspace-container {darkMode ? 'dark' : 'light'}">
	...
	<div class="layout-with-sidebar">
		<aside class="workspace-sidebar {sidebarCollapsed ? 'collapsed' : ''}">
			<button onclick={toggleSidebar} class="sidebar-toggle-btn" title="Toggle sidebar" aria-label="Toggle sidebar">
				<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
					<path d="M4 6h16M4 12h16M4 18h16" />
				</svg>
			</button>
			<nav class="sidebar-nav">
				<a href="/github/workspace/{orgName}/repo-treeview" class="sidebar-link" title="Repo Treeview" aria-label="Repo Treeview">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
						<path d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2H7a2 2 0 00-2 2v2" />
					</svg>
					{#if !sidebarCollapsed}<span>Repo Tree</span>{/if}
				</a>
				<a href="/github/workspace/{orgName}/threat-modeling" class="sidebar-link" title="Threat Modeling" aria-label="Threat Modeling">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
						<path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
					</svg>
					{#if !sidebarCollapsed}<span>Threats</span>{/if}
				</a>
				<a href="/github/workspace/{orgName}/audit" class="sidebar-link" title="Actions Audit" aria-label="Actions Audit">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
						<path d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 002 2h2a2 2 0 012 2" />
					</svg>
					{#if !sidebarCollapsed}<span>Audit</span>{/if}
				</a>
				<a href="/github/workspace/{orgName}/canvas" class="sidebar-link" title="Canvas" aria-label="Canvas">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
						<path d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
					</svg>
					{#if !sidebarCollapsed}<span>Canvas</span>{/if}
				</a>
				<a href="/github/workspace/{orgName}/treeview" class="sidebar-link" title="Treeview" aria-label="Treeview">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
						<path d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z" />
					</svg>
					{#if !sidebarCollapsed}<span>Treeview</span>{/if}
				</a>
				<a href="/github/workspace/{orgName}/predictor" class="sidebar-link active" title="Pipeline Predictor" aria-label="Pipeline Predictor">
					<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
		...
		<main class="workspace-main {sidebarCollapsed ? 'expanded' : ''}">
			<div class="view-header">
				...
				<div class="header-stats">
					<div class="mini-stat">
						<span class="mini-lbl">REPOS</span>
						<span class="mini-val">{workspaceData?.repositories?.length || 0}</span>
					</div>
					<div class="mini-stat">
						<span class="mini-lbl">ACTIVE RUNS</span>
						<span class="mini-val">MONITORING</span>
					</div>
				</div>
			</div>
			...
			{:else if error}
				<div class="error-panel">
					<h3>Failed to load intelligence data</h3>
					<p>{error}</p>
					<button onclick={loadWorkspaceData} class="btn-primary">Retry</button>
				</div>
			{:else}
							...
							<section class="repo-section">
								<div class="repo-header-row">
									...
									<div class="repo-tools">
										<a href={repo.html_url} target="_blank" class="github-link" aria-label="View on GitHub">
...
<style>
	...
	/* Sidebar */
	.layout-with-sidebar { display: flex; flex: 1; min-height: calc(100vh - 104px); }
	.workspace-sidebar {
	...
	.sidebar-toggle-btn {
		...
		padding: 0.5rem; border-radius: 6px; display: flex; justify-content: center;
	}
	...
	.sidebar-link {
		...
		font-size: 0.8125rem; font-weight: 500; transition: all 0.1s;
	}
	.sidebar-link:hover { background: var(--bg-surface-alt); color: var(--text-primary); }
	...
	.repo-icon { width: 20px; height: 20px; color: var(--text-muted); }
	.repo-meta h3 { font-size: 1.125rem; font-weight: 700; letter-spacing: -0.01em; }
	.repo-badge {
		font-size: 0.65rem;
		font-family: var(--font-mono);
	...
	.workflow-node { color: var(--accent); font-family: var(--font-mono); font-weight: bold; }
	.workflow-path { font-weight: 600; font-size: 0.9375rem; }
	.full-path { font-size: 0.7rem; color: var(--text-muted); font-family: var(--font-mono); margin-top: 0.1rem; }
	...
	.shimmer-line {
		height: 12px;
		width: 60%;
		background: linear-gradient(90deg, var(--bg-surface-alt) 25%, var(--border) 50%, var(--bg-surface-alt) 75%);
	...
	.shimmer-panel {
		height: 160px;
		width: 100%;
		background: linear-gradient(90deg, var(--bg-surface-alt) 25%, var(--border) 100%, var(--bg-surface-alt) 75%);
		background-size: 200% 100%;
		animation: shimmer 1.5s infinite;
		border-radius: 8px;
	}
	...
	@media (max-width: 1024px) {
		.workflows-container { grid-template-columns: 1fr; }
	}
</style>
