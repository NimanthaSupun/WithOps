	...
	{#each $others as user (user.id ?? user.presence?.userInfo?.name)}
		{#if user.presence?.cursor && user.presence?.userInfo}
			...
			>
				<svg
					class="cursor-icon"
					style="color: {user.presence.userInfo.color}"
					viewBox="0 0 24 24"
					fill="currentColor"
				>
					<path
						d="M7.4 2.5c-.8 0-1.1.9-.5 1.4l6.5 5.7-3.5 1.4c-.4.2-.5.7-.2 1l2.8 3.5c.3.4.9.4 1.2 0l.9-1.2 2.8.7c.5.1.9-.3.8-.8l-1.2-15.1c-.1-.6-.8-.9-1.3-.5L7.4 2.5z"
					/>
				</svg>
...
{#if showCommentDialog}
	...
	>
		...
		>
			...
			<textarea bind:value={commentText} placeholder="Type your comment here..." rows="3"
			></textarea>
			<div class="comment-actions">
				<button class="cancel-btn" on:click={() => (showCommentDialog = false)}> Cancel </button>
