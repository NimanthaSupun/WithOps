...
<div class="risk-panel {getRiskClass(prediction?.prediction?.risk_level)}">
	...
	<div class="panel-body">
		...
		{:else if !prediction && !loading}
			<div class="empty-msg">Waiting for pipeline data...</div>
		{:else if prediction}
			<div in:fade={{ duration: 300 }}>
				...
				<div class="summary-section">
					<div class="prob-group">
						...
						<div class="prob-value">
							{(prediction.prediction.failure_probability * 100).toFixed(0)}<span class="unit"
								>%</span
							>
						</div>
					</div>
									...
									<div class="factor-content">
										<div class="factor-header">
											...
											<span class="factor-impact"
												>impact: {(factor.importance * 100).toFixed(0)}%</span
											>
										</div>
				...
				<div class="recommendation-box">
					<div class="rec-header">
						<svg class="rec-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"
							><path
...
<style>
	...
	.risk-medium .meter-fill {
		background: #f59e0b;
	}
	.risk-medium .factor-bar {
		background: #f59e0b;
	}
	...
	.risk-critical .meter-fill {
		background: #f43f5e;
	}
	.risk-critical .factor-bar {
		background: #f43f5e;
	}
