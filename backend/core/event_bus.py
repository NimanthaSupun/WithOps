"""
Event Bus for async communication between services using Redis Pub/Sub
"""
import json
import asyncio
import logging
...
class EventBus:
    ...
    async def publish(self, event_type: str, data: Dict[str, Any]):
        ...
        Args:
            event_type: Type of event (e.g., "threat.analysis.requested")
            data: Event payload
        """
        if not self.redis_client:
            await self.connect()
    ...
    async def start_listening(self):
        ...
        # Create pubsub and subscribe to channels
        self.pubsub = self.redis_client.pubsub()
        channels = [f"events:{event_type}" for event_type in self.handlers.keys()]
        await self.pubsub.subscribe(*channels)
        
        logger.info(f"🎧 Listening for events on {len(channels)} channels...")
        self.running = True
...
class TaskQueue:
    ...
    async def get_task_status(self, task_id: str) -> Optional[str]:
        ...
        return await self.redis_client.get(f"task:{task_id}:status")
