...
"""
    ...
    def _parse_stride_analysis(self, text: str) -> Dict[str, Any]:
        ...
        if scope_section:
            sections["scope"] = {
                "in_scope": self._extract_field(scope_section, r"\*\*In-Scope Components:\*\*\s*(.+)"),
                "out_of_scope": self._extract_field(scope_section, r"\*\*Out-of-Scope Components:\*\*\s*(.+)"),
                "assumptions": self._extract_field(scope_section, r"\*\*Assumptions:\*\*\s*(.+)")
            }
        ...
        for i, (category, pattern) in enumerate(zip(stride_categories, stride_patterns)):
            ...
            if category_text:
                ...
                sections["threat_categories"][category] = {
                    "threats": threats,
        ...
        if risk_section:
            sections["risk_rating"] = {
                ...
                "top_risks": self._extract_list_items(risk_section, r"\*\*Top Risks Identified:\*\*\s*((?:\d+\..*?)+)")
            }
        ...
        return {
            ...
            "threat_categories": list(sections.get("stride_analysis", {}).keys()),
            "raw_analysis": text
        }
    ...
    def _parse_cia_analysis(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "methodology": "CIA",
            "parsed_sections": sections,
            "threat_categories": list(sections.get("cia_analysis", {}).keys()),
            "raw_analysis": text
        }
    ...
    def _parse_linddun_analysis(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "methodology": "LINDDUN",
            "parsed_sections": sections,
            "threat_categories": list(sections.get("linddun_analysis", {}).keys()),
            "raw_analysis": text
        }
