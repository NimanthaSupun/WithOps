<script>
    ...
    function parseMethodologyContent(content, methodology = 'STRIDE') {
        ...
        for (const line of lines) {
            ...
            if (trimmedLine.includes('✓ User Identified Threats') || trimmedLine.includes('User Identified Threats')) {
                currentSection = 'userIdentified';
                continue;
            } else if (trimmedLine.includes('🤖 AI Suggested Threats') || trimmedLine.includes('AI Suggested Threats')) {
                currentSection = 'aiSuggested';
                continue;
            } else if (trimmedLine.includes('🛡️ Recommended Mitigations') || trimmedLine.includes('Recommended Mitigations')) {
            ...
            if ((trimmedLine.startsWith('• ') || trimmedLine.startsWith('- ')) && currentSection) {
                ...
                if (item && !result[currentSection].includes(item)) {  // Avoid duplicates
                    result[currentSection].push(item);
                }
            }
        }
    ...
    $: if (showAIAnalysisPanel && !loadingHistory && analysisHistory.length === 0 && !hasInitiallyLoadedHistory) {
            ...
            if (localHistory) {
                ...
                if (Array.isArray(parsedHistory) && parsedHistory.length > 0) {
                    ...
                    if (validHistory.length > 0) {
                        ...
                        console.log('🔍 Analysis IDs:', validHistory.map(h => h.id));
    ...
    $: console.log('🔍 Collaboration state changed:', { collaborationInitialized, currentUser: currentUser ? currentUser.name : 'null' });
        ...
        while (analysisQueue.length > 0) {
            ...
            try {
                ...
                if (error.message.includes('429') || error.message.includes('rate limit')) {
                    ...
                    showUserNotification('⏸️ AI analysis paused due to rate limits. Resuming in 60 seconds...');
    ...
    $: if (g && $canvasData && ($canvasData.elements.length > 0 || $canvasData.connections.length > 0)) {
    ...
    async function initializeCollaborationFeatures() {
        ...
        try {
            ...
            const uniqueUserId = currentUser?.sub ? `auth-${currentUser.sub}` : `user-${sessionId}-${Date.now()}`;
    ...
    const THREAT_METHODOLOGIES = {
        'STRIDE': {
            ...
            elementThreats: {
                rectangle: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                circle: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                square: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                triangle: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                diamond: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                frame: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
                dataflow: ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'],
            },
            threats: {
                'Spoofing': { 
                    color: '#EF4444', 
        ...
        'LINDDUN': {
            ...
            elementThreats: {
                rectangle: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                circle: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                square: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                triangle: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                diamond: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                frame: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance'],
                dataflow: ['Linkability', 'Identifiability', 'Non-repudiation', 'Detectability', 'Disclosure of Information', 'Unawareness', 'Non-compliance']
            },
            threats: {
                'Linkability': { 
                    color: '#EF4444', 
        ...
        'CIA': {
            ...
            elementThreats: {
                ...
                trust_boundary:['Confidentiality Breach', 'Integrity Violation', 'Availability Disruption'],
    ...
    async function loadThreatModel() {
        try {
            ...
            const response = await fetch(`http://localhost:8000/api/threat-modeling/models/${modelId}`, {
            ...
            console.log('🔍 Canvas data keys:', modelData.canvas_data ? Object.keys(modelData.canvas_data) : 'null');
            ...
            } else if (modelData.canvas_data?.metadata?.methodology && THREAT_METHODOLOGIES[modelData.canvas_data.metadata.methodology]) {
            ...
            if (modelData.canvas_data && Object.keys(modelData.canvas_data).length > 0) {
                ...
                console.log('❌ Object.keys(modelData.canvas_data):', modelData.canvas_data ? Object.keys(modelData.canvas_data) : 'null');
    ...
    function updateConnectionsForElement(elementId) {
        ...
        const relatedConnections = data.connections.filter(conn => 
    ...
    function openThreatDialog(element) {
        ...
        console.log('🔵 Final state - showThreatDialog:', $showThreatDialog, 'currentElementForThreat:', $currentElementForThreat);
    ...
    function getCategoryIcon(category) {
        const iconMap = {
            ...
            'Spoofing': '👤', 'spoofing': '👤',
            'Tampering': '⚠️', 'tampering': '⚠️',
            'Repudiation': '📝', 'repudiation': '📝',
            'Information Disclosure': '🔍', 'information_disclosure': '🔍',
            'Denial of Service': '🚫', 'denial_of_service': '🚫',
            'Elevation of Privilege': '⬆️', 'elevation_of_privilege': '⬆️',
            
            // CIA methodology icons
            'Confidentiality Breach': '🔒', 'confidentiality_breach': '🔒',
            'Integrity Violation': '⚖️', 'integrity_violation': '⚖️',
            ...
            'Detectability': '👁️', 'detectability': '👁️',
            'Disclosure of Information': '💡', 'disclosure_of_information': '💡',
            'Unawareness': '❓', 'unawareness': '❓',
            'Non-compliance': '⚖️', 'non_compliance': '⚖️'
        };
    ...
    function getMethodologyDescription(methodology) {
        const descriptions = {
            'STRIDE': 'Microsoft STRIDE methodology focusing on Spoofing, Tampering, Repudiation, Information Disclosure, Denial of Service, and Elevation of Privilege threats',
            'CIA': 'CIA Triad methodology focusing on Confidentiality, Integrity, and Availability of information assets',
            'LINDDUN': 'LINDDUN privacy methodology focusing on Linkability, Identifiability, Non-repudiation, Detectability, Disclosure, Unawareness, and Non-compliance threats'
        };
    ...
    function getCategoryDescription(category, methodology) {
        const descriptions = {
            // STRIDE descriptions
            'Spoofing': 'Identity spoofing attacks where attackers impersonate legitimate users, services, or systems',
            'spoofing': 'Identity spoofing attacks where attackers impersonate legitimate users, services, or systems',
    ...
    function renderCanvas() {
        ...
        try {
            ...
            data.elements.filter(d => d.type === 'frame').forEach((d, index) => {
    ...
    function renderElement(d, index, totalElements) {
        ...
        const group = g.append('g')
                ...
                if (isSelected) {
                    const connectionY = d.type === 'process' ? -Math.min(d.size.width, d.size.height) / 2 - 25 : -d.size.height / 2 - 25;
                        ...
                        .on('mousedown', function(event) {
                            ...
                            function handleMouseMove(moveEvent) {
                                ...
                                const newWidth = Math.max(
                                    elementType.minSize.width,
                                    startWidth + deltaX
                                );
                                const newHeight = Math.max(
                                    elementType.minSize.height,
                                    startHeight + deltaY
                                );
    ...
    function deleteSelectedElement() {
        ...
        canvasData.update(data => {
            ...
            data.connections = data.connections.filter(c => 
                c.source !== selected.id && c.target !== selected.id
            );
            return data;
        });
    ...
    function exportCanvasForAI() {
        return new Promise((resolve, reject) => {
            try {
                ...
                const titleGroup = aiSvg.append('g')
                ...
                titleGroup.append('text')
                    ...
                    .attr('font-size', '12')
                    .attr('fill', '#666666')
                    .text(`Components: ${$canvasData.elements.length} | Connections: ${$canvasData.connections.length} | Threats: ${$canvasData.threats.length}`);
    ...
    function categorizeUserThreatsByFramework(threats, methodology) {
        ...
        threats.forEach(threat => {
            ...
            for (const category of categories) {
                if (threat.type && threat.type.toLowerCase().includes(category.toLowerCase().replace(/\s+/g, ''))) {
        ...
        console.log('✅ Threat categorization complete:', Object.entries(categorizedThreats).map(([cat, threats]) => `${cat}: ${threats.length}`).join(', '));
    ...
    function getKeywordsForCategory(category) {
        const keywordMap = {
            ...
            'Elevation of Privilege': ['privilege', 'escalat', 'unauthorized', 'admin', 'root', 'permission'],
    ...
    async function prepareAIAnalysisData(analysisType = 'comprehensive') {
        try {
            ...
            console.log(`📊 Analysis type: ${actualAnalysisType} (${elements.length} components, ${connections.length} connections)`);
            ...
            if (threats && threats.length > 0) {
                ...
                for (const [category, categoryThreats] of Object.entries(categorizedThreats)) {
                    if (categoryThreats.length > 0) {
                        threatsByCategory[category] = categoryThreats.map(threat => ({
                            ...
                            componentName: elements.find(el => el.id === threat.componentId)?.name || 'Unknown Component',
                ...
                userThreatContext = {
                    ...
                    coverage_summary: Object.entries(threatsByCategory).map(([category, categoryThreats]) => 
                        `${category}: ${categoryThreats.length} threats`
                    ).join(', '),
                    // Analysis hints for AI
                    analysis_notes: {
                        existing_coverage: Object.keys(threatsByCategory),
                        gaps_to_analyze: Object.keys(categorizedThreats).filter(cat => !threatsByCategory[cat] || threatsByCategory[cat].length === 0),
                        components_with_threats: [...new Set(threats.map(t => t.componentId))].length,
                        components_without_threats: elements.length - [...new Set(threats.map(t => t.componentId))].length
                    }
                };
            }
            ...
            const componentsData = elements.map(element => ({
                ...
                user_threats: threats.filter(threat => threat.componentId === element.id).map(threat => ({
                    id: threat.id,
                    title: threat.title,
                    type: threat.type,
                    severity: threat.severity || 'Medium'
    ...
    function loadPastAnalysis(analysis) {
        ...
        tick().then(() => {
            ...
            console.log('✅ aiAnalysisResult.structured_analysis:', aiAnalysisResult?.structured_analysis);
        });
        
        showUserNotification(`📊 Loaded analysis from ${new Date(analysis.timestamp).toLocaleDateString()}`);
    }
    ...
    async function reanalyzeWithReview(category) {
        ...
        try {
            ...
            const reviewContext = {
                ...
                methodology: currentMethodology,
                previous_analysis: aiAnalysisResult.structured_analysis?.parsed_sections?.[getCategoryKey(category)]
            };
    ...
    async function reviewIndividualThreat(threat, category, isValid) {
        try {
            ...
            if (aiAnalysisResult?.structured_analysis?.threat_categories[category]) {
                ...
                const threatIndex = threats.findIndex(t => t.title === threat.title || t.what_can_be_spoofed === threat.what_can_be_spoofed);
                if (threatIndex !== -1) {
    ...
    async function submitThreatReview() {
        ...
        try {
            ...
            if (aiAnalysisResult?.structured_analysis?.threat_categories[currentReviewCategory]) {
                const threats = aiAnalysisResult.structured_analysis.threat_categories[currentReviewCategory].threats;
                const threatIndex = threats.findIndex(t => 
    ...
    async function submitIndividualThreatReview() {
        ...
        try {
            ...
            if (aiAnalysisResult?.structured_analysis?.threat_categories?.[currentThreatCategory]) {
                const categoryData = aiAnalysisResult.structured_analysis.threat_categories[currentThreatCategory];
                ...
                categoryData.threat_reviews = categoryData.threat_reviews.filter(r => r.threat_index !== currentThreatForReview.index);
        ...
        if (analysisResult && typeof analysisResult === 'object' && analysisResult.success) {
            ...
            if (analysisResult.parsed_sections) {
                ...
                return {
                    ...
                    threat_categories: analysisResult.parsed_sections.threat_categories || 
                                     analysisResult.threat_categories || {},
                    
                    // Framework-specific analysis (STRIDE/CIA/LINDDUN)
                    framework_analysis: analysisResult.parsed_sections.stride_analysis || 
    ...
    function parseRawAnalysisText(rawAnalysis, methodology = 'STRIDE') {
        ...
        if (serviceOverviewMatch) {
            ...
            structured.parsed_sections.service_overview = {
                ...
                architecture_summary: extractField(overviewText, /\*\*Architecture Summary:\*\*\s*(.+?)(?=\n|$)/),
        ...
        if (scopeMatch) {
            ...
            structured.parsed_sections.scope = {
                ...
                out_of_scope: extractField(scopeText, /\*\*Out-of-Scope Components:\*\*\s*(.*?)(?=\*\*|$)/s),
        ...
        const mitigationMatch = rawAnalysis.match(/## 5\. (?:Privacy )?Mitigation Plan(.*?)(?=## 6\.|$)/s);
        if (mitigationMatch) {
            const mitigationText = mitigationMatch[1];
            structured.parsed_sections.mitigation_plan = {
                short_term: extractField(mitigationText, /\*\*Short-Term Actions.*?:\*\*\s*(.*?)(?=\*\*|$)/s),
        ...
        try {
            ...
            for (const section of sections) {
                ...
                if (categoryMatch) {
                    ...
                    const userThreatsMatch = section.match(/\*\*✓ User Identified Threats\*\*\s*(.*?)(?=\*\*🔍|$)/s);
                    if (userThreatsMatch) {
                        threatData.user_threats = userThreatsMatch[1].trim();
                    }
                    
                    // Parse AI validation & opinion
                    const aiValidationMatch = section.match(/\*\*🔍 AI Validation & Opinion\*\*\s*(.*?)(?=\*\*🤖|$)/s);
                    if (aiValidationMatch) {
                        threatData.ai_validation = aiValidationMatch[1].trim();
                    }
                    
                    // Parse AI suggested threats  
                    const aiThreatsMatch = section.match(/\*\*🤖 AI Suggested Threats\*\*\s*(.*?)(?=\*\*🛡️|$)/s);
                    if (aiThreatsMatch) {
                        threatData.ai_threats = aiThreatsMatch[1].trim();
                    }
                    
                    // Parse mitigations
                    const mitigationsMatch = section.match(/\*\*🛡️ Recommended Mitigations\*\*\s*(.*?)(?=\*\*|###|$)/s);
                    if (mitigationsMatch) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.detail && errorData.detail.includes('unrelated to threat modeling')) {
                    throw new Error(`❌ Document not suitable for threat modeling:\n\n${errorData.detail}\n\nPlease upload documents like:\n• System architecture diagrams\n• Technical specifications\n• Security requirements\n• API documentation\n• Infrastructure documentation`);
                }
    ...
    async function exportAIAnalysisToPDF() {
        try {
            ...
            try {
                ...
                const response = await fetch('http://localhost:8000/api/ai/claude/export-pdf-url', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            const response = await fetch('http://localhost:8000/api/ai/claude/export-pdf', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function performFreshAIAnalysis() {
        try {
            ...
            console.log('🔍 Debug - modelData structure:', {
                ...
                hasDocumentAnalysis: !!modelData?.document_analysis,
                documentAnalysisKeys: modelData?.document_analysis ? Object.keys(modelData.document_analysis) : [],
                hasExtractedText: !!modelData?.document_analysis?.extracted_text,
            ...
            console.log('📄 Document text for AI analysis:', documentText ? `${documentText.length} characters` : 'No document content');
...
<div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    {:else if error}
        ...
        <div class="flex-1 flex items-center justify-center">
            <div class="text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">Failed to Load Threat Model</h3>
    ...
    {:else}
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <nav class="flex items-center space-x-2 text-sm text-gray-500">
                        ...
                        <span>/</span>
                        <a href={`/github/workspace/${orgName}/threat-modeling`} class="hover:text-gray-700">Threat Modeling</a>
                        <span>/</span>
                        <span class="text-gray-900 font-medium">{modelData?.name || 'Canvas Editor'}</span>
                    </nav>
                </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    {:else}
                        ...
                        <div class="text-xs bg-yellow-100 px-2 py-1 rounded">
                            Debug: collaborationInitialized={collaborationInitialized}, currentUser={currentUser ? 'exists' : 'null'}
                        </div>
                    {/if}
                    ...
                    <button 
                        ...
                        class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:from-purple-700 hover:to-indigo-700 font-medium shadow-sm transition-all duration-200 hover:shadow-md {showAIAnalysisPanel ? 'ring-2 ring-purple-300' : ''}"
                    ...
                    >
                        <div class="flex items-center space-x-2">
                            ...
                            {#if aiAnalysisResult || analysisHistory.length > 0}
                                <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse" title="Has saved analysis"></span>
                            {/if}
                        </div>
                    </button>
        ...
        <div class="flex-1 flex overflow-hidden" style="height: calc(100vh - 80px);">
            <!-- Left Tools Panel - Conditional Display -->

            {#if showDrawingToolsPanel}
            <div class="w-75 bg-white border-r border-gray-200 p-3 flex-shrink-0 overflow-y-auto transition-all duration-300">
                ...
                <div class="space-y-1 max-h-96 overflow-y-auto">
                    {#each Object.entries(ELEMENT_TYPES) as [type, config]}
                        {#if type !== 'dataflow'}
                            <div
                                ...
                                class="w-full flex items-center space-x-2 p-2 border border-gray-200 rounded hover:bg-gray-50 transition-colors duration-200 cursor-grab active:cursor-grabbing group"
                            ...
                            >
                                ...
                                <div class="text-left min-w-0 flex-1">
                                    ...
                                    <div class="text-xs text-gray-500 truncate group-hover:text-gray-700 transition-colors">
                    ...
                    <div
                        ...
                        class="w-full p-2 border-2 border-dashed border-gray-400 rounded hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 cursor-grab active:cursor-grabbing bg-gradient-to-r from-gray-50 to-gray-100"
                        aria-label="Drag to create data flow connection"
                    >
                        <div class="flex items-center justify-center mb-1">
                            <svg width="60" height="20" viewBox="0 0 60 20" class="mx-auto">
                                <line x1="5" y1="10" x2="50" y2="10" stroke="#374151" stroke-width="2" marker-end="url(#dataflow-arrow)"/>
                                <circle cx="5" cy="10" r="3" fill="#22C55E"/>
                                <circle cx="55" cy="10" r="3" fill="#3B82F6"/>
                                <defs>
                                    <marker id="dataflow-arrow" viewBox="0 0 10 10" refX="8" refY="3" markerWidth="6" markerHeight="6" orient="auto">
            ...
            <div class="flex-1 relative min-w-0 transition-all duration-300" style="min-width: 80px;">
                <!-- Canvas Toggle Button - Show when tools panel is hidden -->
                {#if !showDrawingToolsPanel}
                <div class="absolute top-4 left-4 z-10">
                    <button 
                        on:click={() => showDrawingToolsPanel = true}
                        class="bg-white border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm hover:bg-gray-50 shadow-sm transition-all duration-200 hover:shadow-md"
                ...
                {#if showTextInput}
                    <div 
                        class="absolute z-50 bg-white border-2 border-blue-500 rounded-lg shadow-lg p-2"
            ...
            {#if $selectedElement || $selectedConnection}
                ...
                <div class="w-100 bg-white border-l border-gray-200 flex-shrink-0 flex flex-col" style="height: calc(100vh - 80px);">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div class="flex space-x-1">
                                <button 
                                    ...
                                    class="text-blue-600 hover:text-blue-700 px-3 py-1 rounded hover:bg-blue-50 text-sm font-medium border border-blue-300"
                                ...
                                <button 
                                    ...
                                    class="text-white bg-red-600 hover:bg-red-700 px-2 py-1 rounded text-sm font-medium border border-red-600"
                                ...
                                <button 
                                    ...
                                    class="text-gray-600 hover:text-gray-700 px-2 py-1 rounded hover:bg-gray-50 text-sm font-medium border border-gray-300"
                    ...
                    <div class="flex-1 overflow-y-auto p-3 pt-0">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                on:input={() => { renderCanvas(); autoSave(); }}
                        ...
                        {#if !isConnection}
                            ...
                            {#if selectedItem.properties.outOfScope}
                                <div>
                                    <label for="element-reason-out-of-scope" class="block text-sm font-medium text-gray-700 mb-1">Reason</label>
                                    <textarea
                                    ...
                                    {#if threat}
                                        <div class="p-3 bg-red-50 border border-red-200 rounded-lg">
                                            <div class="flex justify-between items-start mb-2">
                                                <div class="flex-1">
                                                    <div class="font-medium text-sm text-red-800">{threat.title}</div>
                                                    <div class="text-xs text-red-600 mb-1">{threat.type}</div>
                                                    ...
                                                    {#if threat.riskLevel}
                                                        <div class="flex items-center space-x-2 mt-1">
                                                            ...
                                                            ></div>
                                                            <span class="text-xs font-medium" style="color: {threat.riskColor || '#EAB308'}">
                                                                {threat.riskLevel} Risk (Score: {threat.riskScore || 'N/A'})
                                                            </span>
                                                        </div>
                                                        <div class="text-xs text-gray-600 mt-1">
                                                            Likelihood: {threat.likelihood || 'N/A'}/5 | Impact: {threat.impact || 'N/A'}/5
                                                        </div>
                                                    {:else}
                                                ...
                                                <div class="flex items-center space-x-1">
                                                    <span class="text-xs bg-red-100 text-red-700 px-2 py-1 rounded">{threat.status}</span>
                                                    <button 
                        ...
                        <div class="pt-3 border-t border-gray-200 text-xs text-gray-500">
                            <div>Type: {isConnection ? 'Connection Line' : ELEMENT_TYPES[selectedItem.type]?.label}</div>
            ...
            {#if showAIAnalysisPanel}
                <div class="bg-white border-l border-slate-200 flex flex-col overflow-hidden relative" 
                     style="width: {aiPanelWidth}px; height: calc(100vh - 80px);">
                    
                    <!-- Resize Handle -->
                    <button 
                        class="absolute left-0 top-0 bottom-0 w-1 bg-slate-200 hover:bg-slate-400 cursor-e-resize z-10 group border-0 p-0"
                        ...
                        <div class="absolute left-0 top-1/2 transform -translate-y-1/2 w-1 h-8 bg-slate-400 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </button>
                    ...
                    {#if currentAIView === 'welcome'}
                        ...
                        <div class="flex flex-col h-full">
                            <!-- Welcome Header -->
                            <div class="p-6 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-blue-50 to-purple-50 flex-shrink-0">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-slate-600">Get comprehensive security insights from Claude AI</p>
                                    </div>
                                </div>
                                <button
                                    on:click={() => showAIAnalysisPanel = false}
                                    class="text-slate-500 hover:text-slate-700 p-2 rounded-lg hover:bg-white/50 transition-colors"
                                >
                                    ✕
                                </button>
                            </div>
                            ...
                            <div class="flex-1 overflow-y-auto p-6 space-y-6">
                                <!-- Welcome Message -->
                                <div class="text-center py-8 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                                    <div class="text-6xl mb-4">🎯</div>
                                    <h3 class="text-2xl font-bold text-slate-900 mb-2">Welcome to AI Threat Analysis</h3>
                                    <p class="text-slate-600 max-w-md mx-auto leading-relaxed">
                                        Upload documents or analyze your current architecture with Claude AI's advanced threat modeling capabilities.
                                    </p>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md transition-shadow">
                                        ...
                                        <p class="text-sm text-slate-600 mb-3">
                                            Upload design documents, architecture diagrams, or requirements for AI analysis
                                        </p>
                                        <input
                                            type="file"
                                        ...
                                        <button
                                            ...
                                            class="w-full bg-green-600 text-white px-4 py-3 rounded-lg font-medium hover:bg-green-700 transition-colors {uploadingDocument ? 'cursor-not-allowed opacity-75' : ''}"
                                        ...
                                        {#if modelData?.document_analysis?.metadata}
                                            <div class="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                                                ...
                                                <div class="text-xs text-green-600 mt-1">{modelData.document_file_name} uploaded successfully</div>
                                            </div>
                                        {/if}
                                    </div>
                                    ...
                                    <div class="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md transition-shadow">
                                        ...
                                        <p class="text-sm text-slate-600 mb-3">
                                            Get AI-powered threat analysis of your current diagram components and connections
                                        </p>
                                        <button 
                                            on:click={() => performAIAnalysis('comprehensive')}
                                            class="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-3 rounded-lg font-medium hover:from-purple-700 hover:to-indigo-700 transition-all {aiAnalyzing ? 'cursor-not-allowed opacity-75' : ''}"
                                        ...
                                        {#if $canvasData.elements.length === 0}
                                            <div class="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
                                                <div class="text-sm text-amber-700">⚠️ Add components to your diagram first</div>
                                            </div>
                                        {/if}
                                    </div>
                                </div>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="space-y-3">
                                            {#each analysisHistory as analysis, index}
                                                <button 
                                                    type="button"
                                                    class="w-full text-left bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer group focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                                                    on:click={() => loadPastAIAnalysis(analysis)}
                                                    on:keydown={(e) => e.key === 'Enter' && loadPastAIAnalysis(analysis)}
                                                    aria-label="Load analysis #{analysisHistory.length - index} from {new Date(analysis.timestamp).toLocaleDateString()}"
                                                    ...
                                                    <div class="flex items-start justify-between">
                                                        <div class="flex-1">
                                                            <div class="flex items-center gap-3 mb-2">
                                                                ...
                                                                <div>
                                                                    <h5 class="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors">
                                                            ...
                                                            <div class="grid grid-cols-3 gap-3 mb-3">
                                                                <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                                    ...
                                                                    <div class="text-sm font-medium text-slate-900">{analysis.methodology || 'STRIDE'}</div>
                                                                </div>
                                                                <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                                    <div class="text-xs text-slate-500">Components</div>
                                                                    <div class="text-sm font-medium text-blue-600">{analysis.diagram_elements_count || 0}</div>
                                                                </div>
                                                                <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                                    <div class="text-xs text-slate-500">Type</div>
                                                                    <div class="text-sm font-medium text-purple-600 capitalize">{analysis.analysis_type || 'comprehensive'}</div>
                                                                </div>
                                                            </div>
                                                            
                                                            <p class="text-sm text-slate-600 leading-relaxed">
                                                                {analysis.analysis?.substring(0, 120) || 'Complete threat analysis with STRIDE methodology'}...
                                                            </p>
                                                        </div>
                                ...
                                {:else if !loadingHistory}
                                    <div class="text-center py-8 text-slate-500">
                                        ...
                                        <h4 class="font-semibold text-slate-900 mb-2">No Past Analysis</h4>
                    ...
                    {:else if currentAIView === 'result'}
                        ...
                        <div class="flex flex-col h-full">
                            <!-- Result Header -->
                            <div class="p-4 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
                                <div class="flex items-center gap-3">
                                    <button
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-600">Claude AI • {aiAnalysisResult?.methodology || currentMethodology} Framework</p>
                                    </div>
                                </div>
                    ...
                    <div class="flex-1 overflow-y-auto bg-slate-50 min-h-0">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            {@const overview = aiAnalysisResult.structured_analysis.parsed_sections.service_overview}
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Service Name</h4>
                                        ...
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Assets to Protect</h4>
                                        ...
                                        <div class="lg:col-span-2">
                                            <h4 class="font-medium text-slate-700 mb-2">Architecture Summary</h4>
                            ...
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        {#if scope.in_scope}
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">In-Scope Components</h4>
                                        ...
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Assumptions</h4>
                                        ...
                                        <div class="lg:col-span-2">
                                            <h4 class="font-medium text-slate-700 mb-2">Out-of-Scope Components</h4>
                            ...
                            {@const riskRating = aiAnalysisResult.structured_analysis.parsed_sections.risk_rating}
                            {#if riskRating.top_risks?.length > 0 && !riskRating.top_risks[0].includes('[') && !riskRating.top_risks[0].includes('Risk 1 with severity')}
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-red-50 to-orange-50">
                                    <h3 class="text-lg font-semibold text-slate-900 flex items-center gap-2">
                                ...
                                <div class="p-6">
                                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Methodology</h4>
                                            <p class="text-slate-600">{riskRating.methodology}</p>
                                        </div>
                                        {/if}
                                        
                                        <div class="lg:col-span-2">
                                            <h4 class="font-medium text-slate-700 mb-2">Top Risks Identified</h4>
                            ...
                            {@const mitigationPlan = aiAnalysisResult.structured_analysis.parsed_sections.mitigation_plan}
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50">
                                    <h3 class="text-lg font-semibold text-slate-900 flex items-center gap-2">
                                        <span class="text-purple-600">🛡️</span>
                                        Mitigation Strategy
                                    </h3>
                                ...
                                <div class="p-6">
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Short-Term Actions</h4>
                                            <div class="text-slate-600 whitespace-pre-line">{mitigationPlan.short_term}</div>
                                        </div>
                                        {/if}
                                        
                                        {#if mitigationPlan.long_term}
                                        <div>
                                            <h4 class="font-medium text-slate-700 mb-2">Long-Term Strategy</h4>
                                            <div class="text-slate-600 whitespace-pre-line">{mitigationPlan.long_term}</div>
                                        </div>
                                        {/if}
                                    </div>
                                </div>
                            ...
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-red-50 to-orange-50">
                                    ...
                                    <p class="text-sm text-slate-600 mt-1">
                                        Review AI-suggested threats and validate their relevance to your architecture
                                    </p>
                                </div>
                                <div class="p-6 space-y-6">
                                    {#each Object.entries(aiAnalysisResult.structured_analysis.threat_categories) as [category, data]}
                                    <div class="border border-slate-200 rounded-lg shadow-sm bg-white">
                                        <!-- Category Header -->
                                        <div class="px-6 py-4 bg-slate-50 border-b border-slate-200">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h4 class="text-lg font-semibold text-slate-800 capitalize">{category.replace(/_/g, ' ')}</h4>
                                                    <p class="text-sm text-slate-600">{getCategoryDescription(category, aiAnalysisResult.methodology || 'STRIDE')}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="p-6 space-y-4">
                                            ...
                                            {#if data.threat_assessment}
                                                <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                                                    ...
                                                    <p class="text-blue-800 leading-relaxed">{data.threat_assessment}</p>
                                                </div>
                                            {/if}
                                            ...
                                            {#if data.user_threats}
                                                <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                                                    ...
                                                    <div class="text-green-800 leading-relaxed whitespace-pre-line">{data.user_threats}</div>
                                                </div>
                                            {/if}
                                            
                                            <!-- AI Validation & Opinion on User Threats -->
                                            {#if data.ai_validation}
                                                <div class="bg-amber-50 rounded-lg p-4 border border-amber-300 shadow-sm">
                                                    ...
                                                    <div class="text-amber-900 leading-relaxed space-y-2">
                                                        {#each data.ai_validation.split('•').filter(v => v.trim()).map(v => v.trim()) as validation}
                                                            <div class="flex items-start gap-2 p-2 bg-white rounded border border-amber-200">
                                                                {#if validation.includes('✅ Valid')}
                                                                    <span class="text-green-600 text-lg flex-shrink-0">✅</span>
                                                                    <div class="flex-1 text-green-800">{validation.replace('✅ Valid:', '').trim()}</div>
                                                                {:else if validation.includes('⚠️ Partially Valid')}
                                                                    <span class="text-orange-600 text-lg flex-shrink-0">⚠️</span>
                                                                    <div class="flex-1 text-orange-800">{validation.replace('⚠️ Partially Valid:', '').trim()}</div>
                                                                {:else if validation.includes('❌ Unlikely')}
                                                                    <span class="text-red-600 text-lg flex-shrink-0">❌</span>
                                                                    <div class="flex-1 text-red-800">{validation.replace('❌ Unlikely:', '').trim()}</div>
                                                                {:else if validation.includes('💡 Enhancement')}
                                                                    <span class="text-blue-600 text-lg flex-shrink-0">💡</span>
                                                                    <div class="flex-1 text-blue-800">{validation.replace('💡 Enhancement:', '').trim()}</div>
                                                                {:else}
                                            ...
                                            {#if data.ai_threats}
                                                <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                                                    ...
                                                    {#each parseAIThreats(data.ai_threats) as threat, threatIndex}
                                                        <div class="mb-3 last:mb-0 bg-white rounded-lg p-4 border border-blue-300 shadow-sm">
                                                            <div class="flex items-start justify-between gap-3">
                                                                <div class="flex-1">
                                                                    <p class="text-blue-900 leading-relaxed">{threat}</p>
                                                                </div>
                                                                
                                                                <!-- Review Button -->
                                                                <button
                                                                    on:click={() => openIndividualThreatReview(threat, category, threatIndex)}
                                                                    class="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium shadow-sm hover:shadow-md"
                                            ...
                                            {#if data.mitigations}
                                                <div class="bg-purple-50 rounded-lg p-4 border border-purple-200">
                                                    ...
                                                    <div class="text-purple-800 leading-relaxed whitespace-pre-line">{data.mitigations}</div>
                                                </div>
                                            {/if}
                                        </div>
                                    </div>
                            ...
                            <!-- Mitigation Plan Section (below threat categories) -->
                            {#if aiAnalysisResult.structured_analysis?.parsed_sections?.mitigation_plan}
                            {@const mitigationPlan = aiAnalysisResult.structured_analysis.parsed_sections.mitigation_plan}
                            {#if mitigationPlan.short_term || mitigationPlan.long_term}
                            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50">
                                    <h3 class="text-lg font-semibold text-slate-900 flex items-center gap-2">
                                        <span class="text-purple-600">🛡️</span>
                            ...
                            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                                <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-yellow-50 to-orange-50">
                                    <h3 class="text-xl font-bold text-slate-900 flex items-center gap-3">
                                        ...
                                        <span class="ml-3 px-3 py-1 bg-yellow-100 text-yellow-800 text-sm rounded-full font-medium">
                                            Basic Parsing Mode
                                        </span>
                                    </h3>
                                    <p class="text-sm text-slate-600 mt-1">
                                        Analysis completed - enhanced parsing with structured sections unavailable
                                    </p>
                                </div>
                                <div class="p-6">
                                    <div class="bg-slate-50 rounded-lg p-6 border border-slate-200">
                                        <pre class="whitespace-pre-wrap text-slate-700 leading-relaxed font-mono text-sm">{aiAnalysisResult.analysis}</pre>
                                    </div>
                                    
                                    <!-- Enhancement Notice -->
                                    <div class="mt-6 p-5 bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl shadow-sm">
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div class="flex-1">
                                                <h4 class="font-bold text-blue-900 mb-2 text-lg">Unlock Enhanced Analysis Features</h4>
                                                <p class="text-sm text-blue-800 mb-4 leading-relaxed">
                                                    Re-run the AI analysis to get structured sections with service overview, scope analysis, risk ratings, 
                                                    mitigation plans, and individual threat review capabilities.
                                                </p>
                                                <button
                                                    on:click={() => performAIAnalysis('comprehensive')}
                                                    class="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg text-sm font-bold hover:from-blue-700 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                        ...
                        {:else}
                            <div class="p-8 text-center text-slate-500">
                                <div class="text-6xl mb-4">🤖</div>
                                <h3 class="text-xl font-semibold text-slate-700 mb-2">No AI Analysis Results</h3>
                                <p class="text-slate-600">Run an AI analysis to see detailed threat assessment and recommendations here.</p>
                            </div>
                        {/if}
                    </div>
            </div>
...
{#if $showEditThreatDialog && $editingThreat}
    ...
    <div class="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-300 shadow-2xl transform transition-transform duration-300 ease-out" 
        ...
        <div class="overflow-y-auto p-6" style="max-height: calc(70vh - 80px);">
            <form on:submit|preventDefault={(e) => {
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="edit-threat-title" class="block text-sm font-medium text-gray-700 mb-1">Title</label>
                            <input
                        ...
                        <div>
                            <label for="edit-threat-type" class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                            <select
                                id="edit-threat-type"
                                name="type"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                            >
                                {#each ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'] as threatType}
                                    <option value={threatType} selected={$editingThreat.type === threatType}>{threatType}</option>
                                {/each}
                            </select>
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <h4 class="text-sm font-semibold text-gray-800 mb-3">🎯 Risk Assessment</h4>
                            
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label for="edit-threat-likelihood" class="block text-sm font-medium text-gray-700 mb-1">
                                        Likelihood
                                    ...
                                    <select
                                        ...
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    >
                                        <option value="1" selected={$editingThreat.likelihood === 1}>Very Low (1)</option>
                                        <option value="2" selected={$editingThreat.likelihood === 2}>Low (2)</option>
                                        <option value="3" selected={$editingThreat.likelihood === 3}>Medium (3)</option>
                                        <option value="4" selected={$editingThreat.likelihood === 4}>High (4)</option>
                                        <option value="5" selected={$editingThreat.likelihood === 5}>Very High (5)</option>
                                    </select>
                                </div>
                                
                                <div>
                                    <label for="edit-threat-impact" class="block text-sm font-medium text-gray-700 mb-1">
                                        Impact
                                    ...
                                    <select
                                        ...
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    >
                                        <option value="1" selected={$editingThreat.impact === 1}>Very Low (1)</option>
                                        <option value="2" selected={$editingThreat.impact === 2}>Low (2)</option>
                                        <option value="3" selected={$editingThreat.impact === 3}>Medium (3)</option>
                                        <option value="4" selected={$editingThreat.impact === 4}>High (4)</option>
                                        <option value="5" selected={$editingThreat.impact === 5}>Very High (5)</option>
                                    </select>
                                </div>
                            </div>
                            
                            <div class="mt-3 p-2 bg-white rounded border border-gray-200">
                                <div class="text-xs text-gray-600 mb-1">Current Risk Level:</div>
                                <div class="flex items-center space-x-2">
                                    <div class="w-4 h-4 rounded-full" style="background-color: {$editingThreat.riskColor || '#EAB308'}"></div>
                                    <span class="text-sm font-medium" style="color: {$editingThreat.riskColor || '#EAB308'}">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="edit-threat-status" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                                <select
                                ...
                                >
                                    ...
                                    <option value="In Progress" selected={$editingThreat.status === 'In Progress'}>In Progress</option>
                                    <option value="Mitigated" selected={$editingThreat.status === 'Mitigated'}>Mitigated</option>
                                    <option value="Accepted" selected={$editingThreat.status === 'Accepted'}>Accepted</option>
                                    <option value="Not Applicable" selected={$editingThreat.status === 'Not Applicable'}>Not Applicable</option>
                                </select>
                            </div>
                            
                            <div>
                                <label for="edit-threat-severity" class="block text-sm font-medium text-gray-700 mb-1">Severity (Legacy)</label>
                                <select
                                ...
                                >
                                    ...
                                    <option value="Medium" selected={$editingThreat.severity === 'Medium'}>Medium</option>
                                    <option value="High" selected={$editingThreat.severity === 'High'}>High</option>
                                    <option value="Critical" selected={$editingThreat.severity === 'Critical'}>Critical</option>
                                </select>
                            </div>
                        </div>
                        
                        <div>
                            <label for="edit-threat-score" class="block text-sm font-medium text-gray-700 mb-1">Score (1-10)</label>
                            <input
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="edit-threat-description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea
                            ...
                            >{$editingThreat.description}</textarea>
                        </div>
                        
                        <div>
                            <label for="edit-threat-mitigation" class="block text-sm font-medium text-gray-700 mb-1">Mitigation</label>
                            <textarea
                        ...
                        <div class="flex justify-end space-x-3 pt-4">
                            <button
                                type="button"
                                on:click={() => showEditThreatDialog.set(false)}
                                class="px-6 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                class="px-6 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 transition-colors shadow-lg"
                            >
                                Update Threat
                            </button>
                        </div>
...
{#if showReviewDialog && currentReviewThreat}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
        <div class="bg-white rounded-lg shadow-2xl border border-slate-200 w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden">
            <!-- Review Header -->
            <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-600">{currentReviewCategory} • Validation Required</p>
                        </div>
                    </div>
                    <button
                        on:click={() => { showReviewDialog = false; currentReviewThreat = null; currentReviewCategory = null; reviewFeedback = ''; reviewValid = null; }}
                        class="text-slate-500 hover:text-slate-700 p-2 rounded-lg hover:bg-white/50 transition-colors"
                    >
                        ✕
                    </button>
            ...
            <div class="p-6 max-h-[60vh] overflow-y-auto">
                <!-- Threat Details -->
                <div class="mb-6">
                    <h4 class="text-md font-semibold text-slate-900 mb-3">Threat Details</h4>
                    <div class="bg-slate-50 rounded-lg p-4 border border-slate-200">
                        <div class="space-y-3">
                            <div>
                                <div class="text-sm font-medium text-slate-700 mb-1">Title</div>
                                <div class="text-sm text-slate-900">
                                    {currentReviewThreat.title || currentReviewThreat.what_can_be_spoofed || 'Security Threat'}
                                </div>
                            </div>
                            <div>
                                <div class="text-sm font-medium text-slate-700 mb-1">Description</div>
                                <div class="text-sm text-slate-600 leading-relaxed">
                                    {currentReviewThreat.description || currentReviewThreat.possible_scenarios || currentReviewThreat.scenarios || 'No description available'}
                                </div>
                            </div>
                            {#if currentReviewThreat.impact && currentReviewThreat.likelihood}
                                <div class="grid grid-cols-3 gap-4 mt-3 p-3 bg-white rounded-lg border border-slate-200">
                                    <div class="text-center">
                                        <div class="text-xs text-slate-500 mb-1">Impact</div>
                                        <div class="text-sm font-bold text-red-600">{currentReviewThreat.impact}/5</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-xs text-slate-500 mb-1">Likelihood</div>
                                        <div class="text-sm font-bold text-orange-600">{currentReviewThreat.likelihood}/5</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-xs text-slate-500 mb-1">Risk Score</div>
                                        <div class="text-sm font-bold text-purple-600">{currentReviewThreat.impact * currentReviewThreat.likelihood}/25</div>
                                    </div>
                                </div>
                            {/if}
                        </div>
                ...
                <!-- Validation Options -->
                <div class="mb-6">
                    <h4 class="text-md font-semibold text-slate-900 mb-3">Is this threat valid?</h4>
                    <div class="flex gap-3">
                        <button
                            on:click={() => reviewValid = true}
                            class="flex-1 px-4 py-3 border border-green-300 rounded-lg hover:bg-green-50 transition-colors {reviewValid === true ? 'bg-green-50 border-green-500 ring-2 ring-green-200' : 'bg-white'}"
                        >
                            <div class="flex items-center justify-center gap-2">
                                <span class="text-green-600 text-lg">✅</span>
                                <span class="font-medium text-green-700">Valid Threat</span>
                            </div>
                            <p class="text-xs text-green-600 mt-1">This threat is legitimate and should be addressed</p>
                        </button>
                        
                        <button
                            on:click={() => reviewValid = false}
                            class="flex-1 px-4 py-3 border border-red-300 rounded-lg hover:bg-red-50 transition-colors {reviewValid === false ? 'bg-red-50 border-red-500 ring-2 ring-red-200' : 'bg-white'}"
                        >
                            <div class="flex items-center justify-center gap-2">
                                <span class="text-red-600 text-lg">❌</span>
                                <span class="font-medium text-red-700">Invalid Threat</span>
                            </div>
                            <p class="text-xs text-red-600 mt-1">This threat is not applicable or incorrect</p>
                        </button>
                    </div>
                </div>
                ...
                <!-- Review Feedback -->
                <div class="mb-6">
                    <label for="review-feedback" class="block text-md font-semibold text-slate-900 mb-3">
                        Review Comments
                    ...
                    <textarea
                        ...
                        class="w-full h-24 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none text-sm"
                    ></textarea>
                    <p class="text-xs text-slate-500 mt-1">
                        Your feedback helps improve AI analysis accuracy and provides context for team members.
                    </p>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button
                            ...
                            class="px-6 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                        >
                            Submit Review
                        </button>
                    </div>
...
{#if showNotification}
    <div class="fixed top-4 right-4 z-50 transition-all duration-300 transform translate-x-0">
        <div class="bg-white border border-gray-200 px-6 py-4 rounded-lg shadow-lg flex items-center space-x-3 max-w-md">
            <!-- Dynamic Icon based on notification content -->
            <div class="flex-shrink-0">
                {#if notification.includes('🤖')}
                    <div class="w-8 h-8 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-full flex items-center justify-center animate-pulse">
                        <span class="text-white text-sm">🤖</span>
                    </div>
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-900 break-words">{notification}</p>
                {#if notification.includes('🤖') && aiAnalyzing}
                    <div class="mt-2">
                        <div class="w-full bg-gray-200 rounded-full h-1.5">
                            <div class="bg-gradient-to-r from-purple-600 to-indigo-600 h-1.5 rounded-full animate-pulse" style="width: 75%"></div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">This may take 10-30 seconds...</p>
                    </div>
                {/if}
            </div>
            ...
            <form on:submit|preventDefault={(e) => {
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="edit-threat-likelihood" class="block text-xs font-medium text-gray-700 mb-1">Likelihood (1-5)</label>
                                <select
                                    id="edit-threat-likelihood"
                                    name="likelihood"
                                ...
                                >
                                    {#each [1,2,3,4,5] as level}
                                        <option value={level} selected={$editingThreat.likelihood == level}>{level}</option>
                                    {/each}
                                </select>
                            </div>
                            
                            <div>
                                <label for="edit-threat-impact" class="block text-xs font-medium text-gray-700 mb-1">Impact (1-5)</label>
                                <select
                                    id="edit-threat-impact"
                                    name="impact"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-red-500"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label for="edit-threat-severity" class="block text-sm font-medium text-gray-700 mb-1">Severity</label>
                            <select
                                id="edit-threat-severity"
                                name="severity"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
...
{#if $showThreatDialog && $currentElementForThreat}
    ...
    <div class="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-300 shadow-2xl transform transition-transform duration-300 ease-out" 
        ...
        <!-- Panel Header - Fixed -->
        <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-red-50 to-orange-50 border-b border-gray-200">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-gradient-to-r from-red-600 to-orange-600 rounded-full flex items-center justify-center shadow-lg">
                    <span class="text-white text-lg">🛡️</span>
                </div>
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Add Security Threat</h3>
                    <p class="text-sm text-gray-600">
                        Component: <span class="font-medium text-blue-600">{$currentElementForThreat.name || $currentElementForThreat.type}</span>
                    </p>
                </div>
            </div>
            <button 
        ...
        <div class="overflow-y-auto p-6" style="max-height: calc(70vh - 80px);">
            <form on:submit|preventDefault={(e) => {
                ...
                <div class="space-y-4">
                    <div>
                        <label for="threat-title" class="block text-sm font-medium text-gray-700 mb-1">Title</label>
                        <input
                            id="threat-title"
                            name="title"
                            type="text"
                    ...
                    <div>
                        <label for="threat-type" class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                        <select
                            id="threat-type"
                            name="type"
                            required
                    ...
                    <div>
                        <label for="threat-severity" class="block text-sm font-medium text-gray-700 mb-1">Severity</label>
                        <select
                            id="threat-severity"
                            name="severity"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    ...
                    <div>
                        <label for="threat-status" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select
                            id="threat-status"
                            name="status"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    ...
                    <div>
                        <label for="threat-description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                        <textarea
                    ...
                    <div>
                        <label for="threat-mitigation" class="block text-sm font-medium text-gray-700 mb-1">Mitigation</label>
                        <textarea
...
{#if showIndividualThreatReview && currentThreatForReview}
    <!-- Overlay -->
    <div class="fixed inset-0 bg-opacity-1 z-40 flex items-center justify-center p-4">
        <!-- Dialog -->
        <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                <!-- Threat Category -->
                <div class="mb-4">
                    <div class="block text-sm font-medium text-gray-700 mb-1">Threat Category</div>
                    <div class="px-3 py-2 bg-gray-50 rounded-lg border border-gray-200">
                        <span class="text-gray-900 font-medium capitalize">{currentThreatCategory?.replace(/_/g, ' ')}</span>
                    </div>
                </div>
                
                <!-- Threat Description -->
                <div class="mb-6">
                    <div class="block text-sm font-medium text-gray-700 mb-2">AI Suggested Threat</div>
                    <div class="px-4 py-3 bg-blue-50 rounded-lg border border-blue-200">
                        <p class="text-blue-900 leading-relaxed">{currentThreatForReview.text}</p>
                    </div>
                </div>
                
                <!-- Validation Section -->
                <div class="mb-6">
                    <div class="block text-sm font-medium text-gray-700 mb-3">Is this threat relevant to your architecture?</div>
                    <div class="flex gap-3">
                        <button
                            on:click={() => threatReviewValid = true}
                            class="flex-1 px-4 py-3 rounded-lg border-2 transition-all {threatReviewValid === true ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 hover:border-green-300'}"
                        >
                            <div class="flex items-center justify-center gap-2">
                                <span class="text-xl">✓</span>
                                <span class="font-medium">Valid</span>
                            </div>
                            <p class="text-sm mt-1 {threatReviewValid === true ? 'text-green-600' : 'text-gray-500'}">This threat applies to our system</p>
                        </button>
                        
                        <button
                            on:click={() => threatReviewValid = false}
                            class="flex-1 px-4 py-3 rounded-lg border-2 transition-all {threatReviewValid === false ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-200 hover:border-red-300'}"
                        >
                            <div class="flex items-center justify-center gap-2">
                                <span class="text-xl">✗</span>
                                <span class="font-medium">Invalid</span>
                            </div>
                            <p class="text-sm mt-1 {threatReviewValid === false ? 'text-red-600' : 'text-gray-500'}">This threat doesn't apply</p>
                        </button>
                    </div>
                </div>
                ...
                <!-- Feedback Section -->
                <div class="mb-6">
                    <label for="threat-review-feedback" class="block text-sm font-medium text-gray-700 mb-2">
